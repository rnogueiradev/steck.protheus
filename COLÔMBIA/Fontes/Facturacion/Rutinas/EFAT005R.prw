...
Private cLogoCli	:=  GetSrvProfString("Startpath","") + "lgrl90.bmp"//"TOTVS.png"	
Private cHoraEnt    := ''
Private cObs        := ''
Private cOrdComp    := ''
Private _obsnLin    := 0
...
Else
	...
	While !ENC->(eof())
		...
		While !DET->(eof())
			...
			oPrint:Say( nLin,nColIni+10 , DET->D2_COD   		, oArial10, 100)
			oPrint:Say( nLin,nColIni+1450 , DET->D2_LOCAL        , oArial10, 100)
			oPrint:Say( nLin,nColIni+1550, TRANSFORM(DET->D2_QUANT,"@E 9,999,999.99")        , oArial10, 100)  
...
//			oPrint:Say( nLin,nColIni+2150, "LOTE123456"	, oArial10, 100)
			...
			/*
			cObs := POSICIONE("SC5",1,XFILIAL("SC5")+DET->D2_PEDIDO,"C5_XOBS")  //Alltrim(SD2->D2_OBS)					
...
Local 	iCont 		:= 1
	...
	oPrint:Say( nRenIni+170,nColIni+800 ," Colombia " + AllTrim(SM0->M0_CEPCOB)	,oArial10N) 
	...
	cHoraEnt := POSICIONE("SC5",1,xFilial("SC5")+cPedido,"C5_XHORA")
	cOrdComp := Alltrim(POSICIONE("SC5",1,xFilial("SC5")+cPedido,"C5_XORDEM"))

	cObs := Alltrim(POSICIONE("SC5",1,XFILIAL("SC5")+cPedido,"C5_XOBS"))  //Alltrim(SD2->D2_OBS)					
	...
	IF SA1->(dbseek(xFILIAL("SA1")+ENC->F2_CLIENTE+ENC->F2_LOJA)) 
		...
		oPrint:Say( nRenIni+650,050,"HOTA ENTREGA: ", oArial10N, 100) 
		oPrint:Say( nRenIni+700,050,"ANEXOS: ", oArial10N, 100) 
		...
		oPrint:Say( nRenIni+650,300,ALLTRIM(cHoraEnt) , oArial10)
		_obsnLin := 0
		...
		if !empty(cObs)    
			...
			For iCont := 1 To nLines
				IF !EMPTY(MemoLine(cObs,,iCont))
					...
					//oPrint:Say(nLin,nColIni+250,ALLTRIM(MemoLine(cObs,,iCont)),oArial10)
					oPrint:Say( nRenIni+700+_obsnLin,300,ALLTRIM(MemoLine(cObs,,iCont)) , oArial10)
					_obsnLin =+ 40
				ENDIF
			Next iCont			
		endif
		...
		oPrint:Say( nRenIni+600,1520,"ORD.COMPRA:	 ", oArial10N, 100) 
		//oPrint:Say( nRenIni+650,1520,"ORDEN COMPRA:   	", oArial10N, 100) 
		...
		oPrint:Say( nRenIni+600 ,1760,cOrdComp       				,oArial10)
...
Static Function PiePag() 
	...
	oPrint:Say (nRenIni+2400+200,nColIni     ,"OBSERVACIONES: "  	,oArial10N, 100 )
