...
Static aCacheOpc    := {}
//Variasveis que verifica o campo RECNO, se é controlado pelo TOTVS DBAcess ou pelo SGBD (2= TOTVS DBAcess, 1= SGBD )
...
@since 17/09/2013
@version P11
@obs Programa cópia do MATA710, reescrito para TOP
/*/
/*-------------------------------------------------------------------*/
...
If !LockByName("MRPEXCL"+cEmpAnt+cFilAnt,.T.,.T.,.T.)
	Help( ,, 'MATA712',, STR0152, 1, 0 ) //"O MRP já está sendo processado por outro usuário. Aguarde o término do processamento para executar o MRP novamente."
	Return
EndIf
...
If AliasInDic("SOQ")
	...
	If SOQ->(dbSeek(xFilial("SOQ")))
	   Help( ,, 'MATA712',, STR0151, 1, 0 )//"MRP Multi-empresas já processado. Execução do MATA712 não permitida. Para realizar o processamento do MRP, utilize o PCPA107."
	   Return
	EndIf
EndIf
If AMIIn(10,44,4)
...
If lRet .And. cIntgPPI != "1"
   ...
   If !Empty(cValue) .And. (cValue != "3" .And. cValue != "30")
      MsgInfo(STR0148,STR0030) //"A integração das ordens de produção com o PC-Factory ainda está em processamento. Execução do MRP não permitida neste momento." "Atenção"
      lRet := .F.
   EndIf
EndIf
...
//Variáveis Tela Geração OPs/SCs
Local aSays    := {}
Local lOpMax99 := .T.
Local nOpcA    := 0
Local nEstmax  := 0
...
Else
				...
				Do Case
					...
					Case GetGlbValue("A712IntPPI"+cEmpAnt+cFilAnt) == '0'
						If nRetry_0 > 50
							...
							Final(STR0166) //"Não foi possivel realizar a subida da thread 'A712IntPPI'"
						Else
							nRetry_0 ++
						EndIf
					...
					Case GetGlbValue("A712IntPPI"+cEmpAnt+cFilAnt) == '20'
						...
						Final(STR0167) //"MATA712: Erro ao efetuar a conexão na thread 'A712IntPPI'"
			...
			While .T.
				Do Case
					...
					Case GetGlbValue(cJobAux) == '0'
						If nRetry_0 > 50
							//Conout(Replicate("-",65))
							//Conout("MATA712: "+ "Não foi possivel realizar a subida da thread" + " " + StrZero(nX,2))
							//Conout(Replicate("-",65))
							...
							Final(STR0161 ) // "Não foi possivel realizar a subida da thread"
						Else
							nRetry_0 ++
						EndIf
	...
	ElseIf lPrcPad .Or. aPergs711[18] == 2
		...
		/*/Filtra o item já com o local do parametro
...
If cIntgPPI != "1"
   ...
   If !Empty(cValue) .And. (cValue != "3" .And. cValue != "30")
      MsgInfo(STR0149+CHR(13)+CHR(10)+ " " + STR0080 + " " + cCount + STR0098 + cTotal,STR0030) //"A integração das ordens de produção com o PC-Factory ainda está em processamento, por favor aguarde." \n Ordem de produção XXX de XXX" "Atenção"
   Else
      oDlg:End()
   EndIf
Else
...
cPesq := cPrd711Vaz+cOpc711Vaz+cRev711Vaz //DMANSMARTSQUAD1-10534 - Corrigido a opção Pesquisa da Tree
...
//					27.nNivExpl		- Nível atual da estrutura que está sendo explodido
//Uso: 		MATA712
//------------------------------------------------------------------------*/
...
If !Empty(cPeriodo)
	//Se o nível não foi passado no parametro, busca da tabela SG1
	If Empty(cNivelEstr)
		SG1->(dbSetOrder(1))
	...
	If !Empty(mOpc) .And. !Empty(cProduto)
		...
		If IsInCallStack("MAT712OPSC")
			If Empty(cOpc) //Quando estiver gerando as ordens, já vem com o cOpc carregado corretamente.
...
//					14.nNivExpl		- Nível atual em que a estrutura está sendo explodida.
...
//Caso exista, é necessário verificar para pegar o R_E_C_N_O_ da CZJ
...
//Autor:		Ricardo Prandi
//Data:		20/09/2013
//Descricao:	Retorna se já existe o registro na CZJ
...
//Se não considera Lote Vencido ou o produto não possui rastro, sai da função
If lConsVenc .Or. !Rastro(cProduto)
	Return 0
EndIf

//Se considera CQ, busca o parâmetro
If aPergs711[22] == 1 .And. cLocCQ == Nil
	cLocCQ := Substr(SuperGetMV("MV_CQ",.F.,"98"),1,2)
EndIf

//Busca posição do período
...
If oSemLocal == Nil
	...
	// Ajuste para banco DB2 - Utilizado após o changeQuery
...
If ! Empty(mOpc)
	cFilSG1 := xFilial("SG1")
	//Opcionais não selecionados - não gerar
...
//				10.nNivExpl		- Nível da estrutura que está sendo explodido.
//				11.cProdFan     - Produto pai do produto fantasma.
//				12.lRecursivo	- Indica que a explosão da estrutura será recursiva. Utilizado para subprodutos que atendem a necessidade produzindo.
//Uso: 		MATA712
//------------------------------------------------------------------------*/
...
//              07.lCalcula     - Indica se será realizado o recálculo das necessidades
//Uso: 			MATA712
//------------------------------------------------------------------------*/
...
//				06.lForcaCalc		- Indica se força o calculo
//				07.lInJob			- Indica se está em JOB
...
//Se calcular estoque de segurança ou ponto de pedido, posiciona a SB1
...
For w := Val(cPeriodo) To Len(aPeriodos)
	...
	If CZK->(dbSeek(cSeek))
		...
		//Ponto de entrada, para alteração das quantidade de Saidas dos produtos
		If lM712Sai
        ...
        //Ponto de Entrada - M712Need para alteração do valor da necessidade
		If lM712Need
...
//Parametros:	cProduto: Produto que será verificado se existem alternativos
//Uso: 		MATA712
//------------------------------------------------------------------------*/
...
While nQuantItem > 0 .And. !(cAliasSGI)->(Eof())
	//Ponto de Entrada para validação da utilização deste alternativo
	If lA712VLALT
...
While nInd <= VAL(cNivMax) //For nInd := 1 to VAL(cNivMax)
	...
	cQuery := " SELECT CZJ.CZJ_PROD, " +;
	           ...
	           " WHERE CZJ.CZJ_FILIAL  = '" + xFilial("CZJ") + "' " +;
	             ...
	             " AND CZK.CZK_QTNECE  > 0 "
	If nInd == 99
		//Só calcula o nível 99 para os produtos que atendem a necessidade produzindo, e são subprodutos em alguma estrutura.
	...
	If Val(cNivMax) == nInd .And. cNivMax != "99"
		/*
			Após calcular todos os níveis, busca os produtos do nível 99 para que caso exista algum produto
			que atenda a necessidade produzindo (campos B1_PRODSBP e B1_ESTRORI) seja calculada a produção desses componentes (função IsNegEstr chamada acima).
		*/
		cNivMax := "99"
		nInd := 98
	EndIf
...
//Recalcula as necessidades do nível 99
			...
			Do Case
				...
				Case GetGlbValue(cJobAux) == '0'
					If nRetry_0 > 50
						...
						//Atualiza o log de processamento
						ProcLogAtu("MENSAGEM","Não foi possivel realizar a subida da thread","Não foi possivel realizar a subida da thread")
						Final(STR0166)  //"Não foi possivel realizar a subida da thread"
					Else
						nRetry_0 ++
					EndIf
				...
				Case GetGlbValue(cJobAux) == '3'
					...
					If File(cFileTotal) .And. ((nHdl := FOpen(cFileTotal,FO_READWRITE)) > 0)
						//Fecha o arquivo para utilizacao da função FT_USE
						fClose(nHd1)
		...
		While .T.
			Do Case
				...
				Case GetGlbValue("A712IntPPI"+cEmpAnt+cFilAnt) == '20'
					...
					//Atualiza o log de processamento
					ProcLogAtu("MENSAGEM","MATA712: Erro ao efetuar a conexão na thread 'A712IntPPI'","MATA712: Erro ao efetuar a conexão na thread 'A712IntPPI'")
					Final(STR0168) // "MATA712: Erro ao efetuar a conexão na thread 'A712IntPPI'"
				Case GetGlbValue("A712IntPPI"+cEmpAnt+cFilAnt) == '2'
					//Thread iniciou processamento, continua a execução do programa.
					//Conout("MATA712: Thread A712IntPPI iniciou o processamento.")
					Exit
...
If Len(aPerOpAuto) > 0
	...
	For nPer:=1 to Len(aPerOpAuto)
		cSelPer := stuff(cSelPer,aPerOpAuto[nPer],1,"û")
	Next nPer
EndIf
...
If Len(aPerScAuto) > 0
	...
	For nPer:=1 to Len(aPerScAuto)
		cSelPerSC := stuff(cSelPerSC,aPerScAuto[nPer],1,"û")
	Next nPer
EndIf
...
Local cProdOP    := ""
Local lGeraNec   := .F. //.T. - Força o MATA650 gerar OP por necessidade -- .F. - (Default) Mantém como está.
Local cGeraOP    := "1" //INDICA A FORMA DE GERAÇÃO DAS OPS DO PRODUTO PAI -- 1 - GERA POR OP, 2 - GERA POR NECESSIDADE
Local aProdOP    := {}
...
If aPergs711[1] == 2
	...
	While !Eof()
		...
		If cTipo711 == "C"
			...
			If ((aPergs711[04] == 1 .And. Substr(cSelPer,nPer,1) == "û") .Or. (aPergs711[04] == 2 .And. Substr(cSelPerSC,nPer,1) == "û")) .AND. A712VerHl(aPeriodos[nPer],cProduto)
		...
		ElseIf cTipo711 == "F"
			...
			If Substr(cSelPer,nPer,1) == "û" .AND. A712VerHl(aPeriodos[nPer],cProduto)
...
While !Eof()
	...
	If cTipo711 = "C" .And. ((aPergs711[04] == 1 .And. Substr(cSelPer,nPer,1) # "û") .Or. (aPergs711[04] == 2 .And. Substr(cSelPerSC,nPer,1) # "û"))
		...
		dbSkip()
		loop
	ElseIf cTipo711 == "F" .And. Substr(cSelPer,nPer,1) # "û"
		dbSelectArea(cAliasTop)
		dbSkip()
		loop
	EndIf
	...
	If (aPergs711[04] == 1 .And. Substr(cSelPer,nPer,1) == "û") .Or. aPergs711[04] == 2
		...
		//Posiciona na CZK para pegar possiveis atualizações de saldo e necessidade
		CZK->(dbGoTo((cAliasTop)->CZKREC))
		nLoteQtd:= CZK->CZK_QTNECE


		/** INICIO ALTERAÇÃO AGLUTINAÇÃO DE OPI/SC **/
			...
			If (cAliasAglu)->(dbSeek(cFilCZI+cProduto+"1"))
				...
				IF (cAliasAglu)->(dbSeek(cFilCZI+cProduto))
					...
					For nZ := 1 to Len(aProdOP)
						...
						EndIf
					Next nZ
					ProcLogAtu("MENSAGEM","OPIs/SCs do componente "+AllTrim(cProduto)+" não aglutinadas.","Os produtos pais "+cProdOp+" estão gerando os documentos por OP.")
				EndIf
			Else
				(cAliasAglu)->(dbSetOrder(1))
				If (cAliasAglu)->(dbSeek(cFilCZI+cProduto))
		...
		/** FIM ALTERAÇÃO AGLUTINAÇÃO DE OPI/SC **/
...
//Atualiza o log de processamento
ProcLogAtu("MENSAGEM","Fim Geração de OPs/SCs","Fim Geração de OPs/SCs")
...
If !IsProdMod(cProduto) .And. ((cAliasTop)->B1_TIPO != "BN" .Or. ((cAliasTop)->B1_TIPO == "BN" .And. MatBuyBN()))
	...
	aCab:={{"C1_EMISSAO",dDataBase  	 		,Nil},; // Data de Emissao
			...
			{"MRPNOVO"   ,"S"						,Nil}}	 //Indica se é o MRP NOVO
...
//				11.lGeraNec   - Indica se força a geração por necessidade
//Uso: 		MATA712
//------------------------------------------------------------------------*/
...
//Autor:		Ricardo Prandi
//Data:		14/10/2013
//Descricao:	Dividir os itens por threads a serem executadas em paralelo para vizualização
...
//Uso: 		MATA712(lFiltro)
//Parâmetro:  .T. -> filtra por necessidade, .F.-> desativa o filtro
//------------------------------------------------------------------------*/
			...
			If nz <= Len(aSaldos)
				...
				If QtdComp(nSaldo) > QtdComp(0)
					...
					For z := nz + 1 To Len(aPeriodos)
						//Pega a qtd. de entradas acumuladas no período.
						nQtdEnt := 0
						For nInd := nZ To z
						...
						While (QtdComp(aSaldos[z,1]) > QtdComp(0)) .And. (QtdComp(nQtdEnt) > QtdComp(0))
							...
							If aSaldos[1,4] > 0
								If QtdComp(aSaldos[1,4]) < QtdComp(aSaldos[z,1])
									...
									aSaldos[1,4] := 0
								Else
									//Zerou a saída com o saldo atual.
							...
							If nAcho > 0
								...
								If !aDados[nAcho,5] //Verifica se já foi gerado LOG do documento. Se sim, apenas atualiza as quantidades.
								...
								If aDados[nAcho,1] > aSaldos[z,1]
									//Se a qtd. do documento é maior que a necessidade do período, desconta a qtd. do documento.
...
@author lucas.franca
@since 01/02/2018
@param cProduto	- Código do produto para buscar os opcionais
...
Static Function A712OpcDft(cProduto,aOpc)
	...
	//Se não tiver nenhum opcional informado, busca o opcional default do produto.
	//Se existir opcional default na SB1, ele será considerado.
...
Static Function MT712Autom(lFlag)
	//Limpa as variáveis estáticas do fonte MATA712, para garantir a integridade da automação.
	clrStatic()
	lIsADVPR := lFlag
Return
...
--copiado fo fonte sigacusb.prx funcao e adaptado para função MT712VLOPC
@author thiago.zoppi
@since 10/05/2018
@version 1.0
/-------------------------------------------------------------------------*/
...
--copiado do fonte sigacusb.prx e adaptado para função  M712OpcOk
@author thiago.zoppi
@since 10/05/2018
@version 1.0
/-------------------------------------------------------------------------*/
...
Monta query com a lógica para recuperar o número do período do MRP.

@author lucas.franca
@since 24/05/2018
@version 1.0

@param aPeriodos	- Array com os períodos do MRP.
@param cCampoDate	- Campo que contém a informação da data. Ex: "SC1.C1_DATPRF"
@param dDatFim		- Data final a ser considerada no processamento do MRP.

@return cSqlPeriod	- Query com a lógica para recuperar o período do MRP.
/*/
Static Function gSqlPerMRP(aPeriodos, cCampoDate, dDatFim)
	...
	//Verifica se a data está entre o último período e a data final do MRP, devendo considerar o último período.
	...
	cSqlPeriod += " ELSE '   ' " //Nenhum período válido, retorna branco.
	cSqlPeriod += " END "
Return cSqlPeriod

/*/{Protheus.doc} gSqlNxtUti
Monta query com a lógica para recuperar o próximo dia útil.
Lógica será utilizada quando o parâmetro 12 do MRP estiver diferente de 1 (não considera finais de semana)
...
@return cSqlNxtUti	- Query com a lógica para recuperar a próxima data útil.
/*/
Static Function gSqlNxtUti(cCampoDate)
	...
	Else
		...
		//não precisa fazer o cálculo da data.
...
Monta query com a lógica para somar dias em um campo de data, que é salvo no banco de dados no formato string (AAAAMMDD).

@author lucas.franca
@since 07/06/2018
@version 1.0

@param cCampo	- Campo que contém a informação da data. Ex: "SC1.C1_DATPRF"
@param nDias	- Quantidade de dias que serão somados no campo de data.

@return cSqlSomaDt	- Query com a lógica para recuperar a próxima data útil.
/*/
...
Monta query com a lógica para recuperar o nível do produto.

@author lucas.franca
@since 25/05/2018
@version 1.0

@param cCampoProd	- Campo que contém a informação do código do produto. Ex: "SC2.C2_PRODUTO"

@return cSqlNivel	- Query com a lógica para recuperar o período do MRP.
/*/
...
Gera query SQL para geração do campo R_E_C_N_O_ das tabelas

@param cTable	- Tabela que será gerado a numeração do RECNO
...
@param cQueryB1		- Variável por referência, para recuperar a query a ser utilizada nos processos da SB1.
...
@param cAlmoxd		- Armazém de - para filtro dos produtos.
@param cAlmoxa		- Armazém até - para filtro dos produtos.
@param aPeriodos	- Períodos de processamento do MRP.
...
Static Function prcSldIni(cQueryB1,lAllTp,lAllGrp,cAlmoxd,cAlmoxa,aPeriodos,cStrTipo,cStrGrupo)
	...
	/*
		Inserir todos os produtos na CZJ, mesmo que não tenham saldo.
		Inserir as quantidades de saldo na CZK para o primeiro período.
		Se tiver estoque de segurança ou ponto de pedido, inserir na CZI e atualizar a CZK.
		Se existir produtos com fórmula no estoque de segurança, deverá ser realizado loop no ADVPL para fazer o cálculo do estoque de segurança.
	*/
	...
	//Necessário executar a TCRefresh para atualizar o cache do DBACCESS referente a tabela CZJ.
	//Se não for chamada esta função, ocorrerá erro ao realizar uma insersão por RecLock.
	...
	If aPergs711[26] == 1
		...
		//Executa a query.
		If TcSqlExec(cInsert) < 0
			Final("Erro na carga do estoque de segurança dos produtos do MRP.", TCSQLError() + cInsert)
		EndIf
		...
		If TcSqlExec(cUpdate) < 0
			Final("Erro na carga da tabela CZK - Estoque de segurança (CZK_QTSLES). ", TCSQLError() + cInsert)
		EndIf
		//Necessário executar a TCRefresh para atualizar o cache do DBACCESS referente a tabela CZJ.
		//Se não for chamada esta função, ocorrerá erro ao realizar uma insersão por RecLock.
		...
		//Processa o estoque de segurança para os produtos que utilizam fórmula (B1_ESTFOR)
		cArqTrb := "BUSCESTSEG"
		SB1->(dbSetOrder(1))
	...
	If aPergs711[31] == 1
		...
		//Necessário executar a TCRefresh para atualizar o cache do DBACCESS referente a tabela CZI.
		//Se não for chamada esta função, ocorrerá erro ao realizar uma insersão por RecLock.
		TCRefresh(RetSqlName("CZI"))
	EndIf
Return Nil
...
Busca os dados das solicitações de compra para carregar a tabela CZI.
@author lucas.franca
@since 21/05/2018
@version 1.0

@param cQueryB1		- Query para filtro dos produtos (SB1)
@param cAlmoxd		- Armazém de - para filtro das solicitações de compra.
@param cAlmoxa		- Armazém até - para filtro das solicitações de compra.
@param dDatFim		- Data final para filtro das solicitações de compra.
@param aPeriodos	- Array com os períodos do MRP.
...
Static Function procSC1(cQueryB1,cAlmoxd,cAlmoxa,dDatFim,aPeriodos,lConsSusp,lConsSacr)
	...
	//Executa a query.
	If TcSqlExec(cInsert) < 0
		Final("Erro na carga das solicitações de compra. ", TCSQLError() + cInsert)
	EndIf
...
Busca os dados das ordens de produção para carregar a tabela CZI.
@author lucas.franca
@since 21/05/2018
@version 1.0

@param cQueryB1		- Query para filtro dos produtos (SB1)
@param cAlmoxd		- Armazém de - para filtro das ordens de produção.
@param cAlmoxa		- Armazém até - para filtro das ordens de produção.
@param dDatFim		- Data final para filtro das ordens de produção.
@param aPeriodos	- Array com os períodos do MRP.
@param lConsSusp	- Indicador para considerar ordens suspensas
@param lConsSacr	- Indicador para considerar ordens sacramentadas
...
Static Function procSC2(cQueryB1,cAlmoxd,cAlmoxa,dDatFim,aPeriodos,lConsSusp,lConsSacr)
	...
	//Executa a query.
	If TcSqlExec(cInsert) < 0
		Final("Erro na carga das ordens de produção. ", TCSQLError() + cInsert)
	EndIf
...
@param cQueryB1		- Query para filtro dos produtos (SB1)
@param cAlmoxd		- Armazém de - para filtro dos empenhos.
@param cAlmoxa		- Armazém até - para filtro dos empenhos.
...
Static Function procSD4(cQueryB1,cAlmoxd,cAlmoxa,dDatFim,aPeriodos,lConsSusp,lConsSacr)
	...
	If cBanco == "POSTGRES"
		ProcLogAtu("MENSAGEM","Iniciando criação do indice GET_OP","Iniciando criação do indice GET_OP")

		cNameIndex := RetSqlName("SC2")+"_get_op"
		cQuery := "CREATE INDEX "+cNameIndex+" ON "+RetSqlName("SC2")+" ((C2_NUM||C2_ITEM||C2_SEQUEN||C2_ITEMGRD))"
		TcSqlExec(cQuery)

		ProcLogAtu("MENSAGEM","Termino criação do indice GET_OP","Termino criação do indice GET_OP")
	EndIf
...
Busca os dados dos pedidos de compra/autorização de entrega para carregar a tabela CZI.
@author lucas.franca
@since 04/06/2018
@version 1.0

@param cQueryB1		- Query para filtro dos produtos (SB1)
@param cAlmoxd		- Armazém de - para filtro dos pedidos de compra/autorização de entrega.
@param cAlmoxa		- Armazém até - para filtro dos pedidos de compra/autorização de entrega.
@param dDatFim		- Data final para filtro dos pedidos de compra/autorização de entrega.
@param aPeriodos	- Array com os períodos do MRP.
@param lConsSusp	- Indicador para considerar ordens suspensas
@param lConsSacr	- Indicador para considerar ordens sacramentadas
...
Static Function procSC7(cQueryB1,cAlmoxd,cAlmoxa,dDatFim,aPeriodos,lConsSusp,lConsSacr)
	...
	If TcSqlExec(cInsert)
		Final("Erro na carga dos pedidos de compra/autorização de entrega. ", TCSQLError() + cInsert)
	EndIf
...
Busca os dados do plano mestre de produção para carregar a tabela CZI.
@author lucas.franca
@since 07/06/2018
@version 1.0
...
Static Function procSHC(cQueryB1,dDatFim,aPeriodos)
	...
	//Executa a query.
	If TcSqlExec(cInsert)
		Final("Erro na carga do plano mestre de produção. ", TCSQLError() + cInsert)
	EndIf
...
@param cQueryB1		- Query para filtro dos produtos (SB1)
@param cAlmoxd		- Armazém de - para filtro dos lotes.
@param cAlmoxa		- Armazém até - para filtro dos lotes.
...
@param cQueryB1		- Query para filtro dos produtos (SB1)
@param cAlmoxd		- Armazém de - para filtro dos pedidos de venda.
@param cAlmoxa		- Armazém até - para filtro dos pedidos de venda.
...
Monta a cláusula WHERE padrão para busca de dados na tabela SC6.
...
@param cComp1		- Filtro para busca de pedidos bloqueados (C6_BLQ).
@param cComp2		- Filtro para busca de pedidos bloqueados (C6_BLQ).
@param lVldQtd		- Indica se será aplicado o filtro de quantidade faturada.

@return cWhere	- Condição SQL para filtro da tabela SC6.
/*/
...
Busca os dados dos previsões de venda para carregar a tabela CZI.
...
@param cQueryB1		- Query para filtro dos produtos (SB1)
@param cAlmoxd		- Armazém de - para filtro das previsões de venda.
@param cAlmoxa		- Armazém até - para filtro das previsões de venda.
@param dDatFim		- Data final para filtro das previsões de venda.
@param aPeriodos	- Array com os períodos do MRP.
@param cComp1		- Filtro para busca de pedidos bloqueados (C6_BLQ).
@param cComp2		- Filtro para busca de pedidos bloqueados (C6_BLQ).
...
USER Function procSC4(cQueryB1,cAlmoxd,cAlmoxa,dDatFim,aPeriodos,cComp1,cComp2,nTipo)
	...
	//Executa a query.
	If TcSqlExec(cInsert)
		Final("Erro na carga das previsões de venda. ", TCSQLError() + cInsert)
	EndIf
	...
	//Atualiza a quantidade das previsões de venda, de acordo com os pedidos de venda.
		...
		While (cAliasC4)->(!Eof())			
			...
			While (cAliasC6)->(!Eof())
				...
				If aPedidos[nPos,2] >= CZI->CZI_QUANT
					...
					If TcSqlExec(cQuery)
						Final("Erro na carga das previsões de venda. ", TCSQLError() + cQuery)
					EndIf
					Exit
				Else
					RecLock("CZI",.F.)
...
Monta a cláusula WHERE padrão para relacionamento dos campos C4_DATA e C6_ENTREG
...
@return cWhere	- Condição SQL para relacionamento dos campos C4_DATA e C6_ENTREG.
/*/
	...
	Do Case
	    ...
	    Case nTipo == 2 //Semanal
			If cBanco $ "POSTGRES|ORACLE"
				...
				cWhere +=   " OR TO_CHAR(TO_DATE(SC4.C4_DATA,'YYYYMMDD'), 'IYYY-IW') = TO_CHAR(TO_DATE(SC6.C6_ENTREG,'YYYYMMDD'), 'IYYY-IW')) " //Compara se a data da SC4 e SC6 estão na mesma semana.
			Else
				cWhere += cQryAno //Compara o ANO da SC6 e da SC4
				cWhere +=   " AND DATEPART(WEEK, CAST(SC4.C4_DATA AS DATE)) = DATEPART(WEEK, CAST(SC6.C6_ENTREG AS DATE))) " // Compara se a data da SC4 e SC6 estão na mesma semana do ano.
			EndIf
	    Case nTipo == 3 //Quinzenal
	    	cWhere += cQryAno //Compara o ANO da SC6 e da SC4
			cWhere += cQryMes //Compara o MÊS da SC6 e da SC4
			If cBanco == "POSTGRES"
				cWhere +=   " AND ((DATE_PART('DAY', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 1  AND 15 "  //Compara se os dias da SC6 e SC4 estão na primeira quinzena do mês
    			cWhere +=   " AND   DATE_PART('DAY', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 1  AND 15) " //Compara se os dias da SC6 e SC4 estão na primeira quinzena do mês
				cWhere +=   "  OR  (DATE_PART('DAY', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 16 AND 31 "   //Compara se os dias da SC6 e SC4 estão na segunda quinzena do mês
				cWhere +=   " AND   DATE_PART('DAY', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 16 AND 31)) " //Compara se os dias da SC6 e SC4 estão na segunda quinzena do mês
			ElseIf cBanco == "ORACLE"
				cWhere +=   " AND ((EXTRACT(DAY FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 1  AND 15 "  //Compara se os dias da SC6 e SC4 estão na primeira quinzena do mês
				cWhere +=   " AND   EXTRACT(DAY FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 1  AND 15) " //Compara se os dias da SC6 e SC4 estão na primeira quinzena do mês
				cWhere +=   "  OR  (EXTRACT(DAY FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 16 AND 31 "   //Compara se os dias da SC6 e SC4 estão na segunda quinzena do mês
				cWhere +=   " AND   EXTRACT(DAY FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 16 AND 31)) " //Compara se os dias da SC6 e SC4 estão na segunda quinzena do mês
			Else
				cWhere +=   " AND ((DAY(CAST(SC4.C4_DATA   AS DATE)) BETWEEN 1  AND 15 "  //Compara se os dias da SC6 e SC4 estão na primeira quinzena do mês
				cWhere +=   " AND   DAY(CAST(SC6.C6_ENTREG AS DATE)) BETWEEN 1  AND 15) " //Compara se os dias da SC6 e SC4 estão na primeira quinzena do mês
				cWhere +=   "  OR  (DAY(CAST(SC4.C4_DATA   AS DATE)) BETWEEN 16 AND 31 "   //Compara se os dias da SC6 e SC4 estão na segunda quinzena do mês
				cWhere +=   " AND   DAY(CAST(SC6.C6_ENTREG AS DATE)) BETWEEN 16 AND 31)) " //Compara se os dias da SC6 e SC4 estão na segunda quinzena do mês
			EndIf
			cWhere += " ) "
		Case nTipo == 4 //Mensal
			cWhere += cQryAno //Compara o ANO da SC6 e da SC4
		...
		Case nTipo == 5 //Trimestral
			cWhere += cQryAno //Compara o ANO da SC6 e da SC4
			If cBanco == "POSTGRES"
				cWhere +=   " AND ((DATE_PART('MONTH', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 1   AND 3 "    //Compara se os dias da SC6 e SC4 estão no primeiro trimestre do ano
    			cWhere +=   " AND   DATE_PART('MONTH', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 1   AND 3) "   //Compara se os dias da SC6 e SC4 estão no primeiro trimestre do ano
    			cWhere +=   "  OR  (DATE_PART('MONTH', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 4   AND 6 "    //Compara se os dias da SC6 e SC4 estão no segundo trimestre do ano
    			cWhere +=   " AND   DATE_PART('MONTH', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 4   AND 6) "   //Compara se os dias da SC6 e SC4 estão no segundo trimestre do ano
    			cWhere +=   "  OR  (DATE_PART('MONTH', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 7   AND 9 "    //Compara se os dias da SC6 e SC4 estão no terceiro trimestre do ano
    			cWhere +=   " AND   DATE_PART('MONTH', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 7   AND 9) "   //Compara se os dias da SC6 e SC4 estão no terceiro trimestre do ano
				cWhere +=   "  OR  (DATE_PART('MONTH', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 10  AND 12 "   //Compara se os dias da SC6 e SC4 estão no quarto trimestre do ano
				cWhere +=   " AND   DATE_PART('MONTH', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 10  AND 12))) " //Compara se os dias da SC6 e SC4 estão no quarto trimestre do ano
			ElseIf cBanco == "ORACLE"
				cWhere +=   " AND ((EXTRACT(MONTH FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 1  AND 3 "     //Compara se os dias da SC6 e SC4 estão no primeiro trimestre do ano
				cWhere +=   " AND   EXTRACT(MONTH FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 1  AND 3) "    //Compara se os dias da SC6 e SC4 estão no primeiro trimestre do ano
				cWhere +=   "  OR  (EXTRACT(MONTH FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 4  AND 6 "     //Compara se os dias da SC6 e SC4 estão no segundo trimestre do ano
				cWhere +=   " AND   EXTRACT(MONTH FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 4  AND 6) "    //Compara se os dias da SC6 e SC4 estão no segundo trimestre do ano
				cWhere +=   "  OR  (EXTRACT(MONTH FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 7  AND 9 "     //Compara se os dias da SC6 e SC4 estão no terceiro trimestre do ano
				cWhere +=   " AND   EXTRACT(MONTH FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 7  AND 9) "    //Compara se os dias da SC6 e SC4 estão no terceiro trimestre do ano
				cWhere +=   "  OR  (EXTRACT(MONTH FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 10  AND 12 "   //Compara se os dias da SC6 e SC4 estão no quarto trimestre do ano
				cWhere +=   " AND   EXTRACT(MONTH FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 10  AND 12))) " //Compara se os dias da SC6 e SC4 estão no quarto trimestre do ano
			Else
				cWhere +=   " AND DATEPART(QUARTER, CAST(SC4.C4_DATA AS DATE)) = DATEPART(QUARTER, CAST(SC6.C6_ENTREG AS DATE))) " // Compara se a data da SC4 e SC6 estão no mesmo trimestre do ano.
			EndIf
		Case nTipo == 6 //Semestral
			cWhere += cQryAno //Compara o ANO da SC6 e da SC4
			If cBanco == "POSTGRES"
			   	cWhere +=   " AND ((DATE_PART('MONTH', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 1  AND 6 "    //Compara se os dias da SC6 e SC4 estão no primeiro semestre do ano
    			cWhere +=   " AND   DATE_PART('MONTH', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 1  AND 6) "   //Compara se os dias da SC6 e SC4 estão no primeiro semestre do ano
				cWhere +=   "  OR  (DATE_PART('MONTH', TO_DATE(SC4.C4_DATA,'YYYYMMDD'))   BETWEEN 7  AND 12 "   //Compara se os dias da SC6 e SC4 estão no segundo semestre do ano
				cWhere +=   " AND   DATE_PART('MONTH', TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 7  AND 12)) " //Compara se os dias da SC6 e SC4 estão no segundo semestre do ano
			ElseIf cBanco == "ORACLE"
			   	cWhere +=   " AND ((EXTRACT(MONTH FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 1  AND 6 "    //Compara se os dias da SC6 e SC4 estão no primeiro semestre do ano
				cWhere +=   " AND   EXTRACT(MONTH FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 1  AND 6) "   //Compara se os dias da SC6 e SC4 estão no primeiro semestre do ano
				cWhere +=   "  OR  (EXTRACT(MONTH FROM TO_DATE(SC4.C4_DATA  ,'YYYYMMDD')) BETWEEN 7  AND 12 "   //Compara se os dias da SC6 e SC4 estão no segundo semestre do ano
				cWhere +=   " AND   EXTRACT(MONTH FROM TO_DATE(SC6.C6_ENTREG,'YYYYMMDD')) BETWEEN 7  AND 12)) " //Compara se os dias da SC6 e SC4 estão no segundo semestre do ano
			Else
				cWhere +=   " AND ((DATEPART(MONTH, CAST(SC4.C4_DATA AS DATE))   BETWEEN 1  AND 6 "    //Compara se os dias da SC6 e SC4 estão no primeiro semestre do ano
				cWhere +=   " AND   DATEPART(MONTH, CAST(SC6.C6_ENTREG AS DATE)) BETWEEN 1  AND 6) "   //Compara se os dias da SC6 e SC4 estão no primeiro semestre do ano
				cWhere +=   "  OR  (DATEPART(MONTH, CAST(SC4.C4_DATA AS DATE))   BETWEEN 7  AND 12 "   //Compara se os dias da SC6 e SC4 estão no segundo semestre do ano
				cWhere +=   " AND   DATEPART(MONTH, CAST(SC6.C6_ENTREG AS DATE)) BETWEEN 7  AND 12)) " //Compara se os dias da SC6 e SC4 estão no segundo semestre do ano
				cWhere += " ) "
			EndIf
...
Cria os registros necessários na tabela CZJ e CZK, com base nos dados existentes na tabela CZI.
Utilizado somente para alimentar a tabela CZJ e CZK quando for utilizado o comando INSERT/SELECT para criação da CZI.
...
Static Function procOpcMrp()
	...
	While (cAlias)->(!Eof())
		...
		//Necessário tratar aqui para o banco POSTGRES, que salva o campo MEMO com valores no banco, mesmo que seja valor NULL.
...
Cria os registros necessários na tabela CZK, com base nos dados existentes na tabela CZI.
Utilizado somente para alimentar a tabela CZK quando for utilizado o comando INSERT/SELECT para criação da CZI.
...
Static Function procCZK()
	...
	Local cCondUpd  := ""
	Local cBanco    := AllTrim(Upper(TcGetDb()))
	//Primeiro alimenta a tabela CZK com quantidade zerada, para os períodos que existem
	//na tabela CZI mas não existem na tabela CZK.
	...
	//Executa a query.
	If TcSqlExec(cUpdate) < 0
		Final("Erro na carga da tabela CZK - Saídas (CZK_QTSAID). ", TCSQLError() + cInsert)
	EndIf

	//Necessário executar a TCRefresh para atualizar o cache do DBACCESS referente a tabela CZK.
	//Se não for chamada esta função, ocorrerá erro ao realizar uma insersão por RecLock.
	TCRefresh(RetSqlName("CZK"))
Return Nil
...
Recupera a forma de geração do MRP de determinado produto (B5_FORMMRP)
...
@return cRet - Conteúdo do campo B5_FORMMRP do produto. Valor padrão é 1.

@param cProduto, characters, código do produto para buscar o valor do B5_FORMMRP
...
Recupera a forma de aglutinação do MRP de determinado produto (B5_AGLUMRP)
@author lucas.franca
@since 05/06/2018
@version 1.0
@return cRet - Conteúdo do campo B5_AGLUMRP do produto. Valor padrão é 1.
@param cProduto, characters, código do produto para buscar o valor do B5_AGLUMRP
/*/
...
Limpa as variáveis do tipo Static deste fonte. Necessário para garantir integridade dos scripts de Automação.
...
Nova função a ser utilizado na rotina de LOG do MRP
@author Thiago.Zoppi
@since 21/11/2018
@version 1.0
/*/
...
//	cQuery := ChangeQuery(cQuery)
	...
	While ! (cAliasTop)->(Eof())
		...
		IF (cAliasTop)->CZK_QTENTR > 0  // TEM ENTRADAS
			...
			//EXISTEM MAIS QUE 1 DOC PARA O PERIODO OU A NECESSIDADE É IGUAL A ZERO, E EXISTEM ENTRADAS
			...
			ELSE
				//-------------------------------------------------------//
				// -- DISTRIBUIR OS DOCS QUE NÃO FORAM UTILIZADOS        //
				//-------------------------------------------------------//
				IF oSqlLogDoc == nil
				...
				While (cAlias)->(!eof())
					...
					IF  EMPTY((cAlias01)->CZK_PERMRP) // NAO EXISTEM REGISTROS
						//Nao existem necessidades - então cancela o Doc
					...
					IF  nValor >= (cAlias01)->CZK_QTNECE
						...
						IF nValor > 0 //sobrou saldo, atualizar saldos
							...
							// verifica se o periodo do documento é o mesmo da CZK
...
Static Function AtuOPSC2(cError)
    ...
    ProcLogAtu("MENSAGEM","Iniciando Atualização C2_OP","Iniciando Atualização C2_OP")
	...
	ProcLogAtu("MENSAGEM","Termino Atualização C2_OP","Fim Atualização C2_OP")
Return lRet
...
/*/{Protheus.doc} A712Lote(nQtdTotal,cProduto)
    ...
    @since 01/05/2019
    @version 1.0
    @param nQtdTotal, Numérico, Quantidade total do produto
...
Verifica se um índice existe para a tabela.

@type  Static Function
@author lucas.franca
@since 03/04/2020
@version P12.1.30
@param cNameIndex, Character, Nome do índice no banco de dados
@param cTable    , Character, Nome da tabela
@return lExiste, Logic, Indica se o índice existe ou não.
/*/
