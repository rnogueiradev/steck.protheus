...
±±º Programa    ³ TESTABLE ³ Efetua manutenção na tabela DJ do arquivo SX5                º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º Autor       ³ 13.06.13 ³ João Victor                                                  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
User Function TESTABLE(cTabela)
	//Local aTabelas := {}
	//Local oDlgTabe
	Local nq        	  := 0
	Local n        	  := 0
	...
	If !dbSeek(xFilial("SX5")+"00"+_cTabela)
		MsgStop("O cabeçalho da tabela "+ _cTabela +" não foi encontrado. Cadastre a tabela DJ(SX5) para prosseguir.","Atenção","STOP" )
		Return
	EndIf
	...
	#ELSE
	Aadd(aC,{"_cChave" ,{06,10} ,"Tabela"   ,"@!"," ","",.f.})
	Aadd(aC,{"_cDescri",{06,58} ,"Descricao","@!"," ","",.f.})
	#ENDIF
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	...
	lRetMod2 := .F.
	//N        := 1
	lRetMod2 := Modelo2(cTitulo,aC,aR,aCGD,nOpcx,cLinhaOk,cTudoOk,{"X5_CHAVE","X5_DESCRI"},,,999,,,.T.,)
