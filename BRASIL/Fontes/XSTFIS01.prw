...
For n0 := 1 to Len(aSM0)
	...
	#IFDEF TOP
		If TcSrvType() <> "AS/400"
			...
			If !lRelDIPAM .and. !lTms
				...
				cQuery += "SF3.F3_CLIEFOR,SF3.F3_LOJA,SF3.F3_EMISSAO, SF3.F3_TIPO,"						
			...
			Else
				...
				cQuery := "SELECT SF3.F3_FILIAL,SF3.F3_ENTRADA,SF3.F3_DTCANC,SF3.F3_CREDST, "
			...
			IF !lRelDIPAM .AND. !lTms			
				...
				cQuery += "SF3.F3_CLIEFOR,SF3.F3_LOJA,SF3.F3_EMISSAO,  "						
				cQuery += "SA1.A1_TIPO,SA1.A1_MUN ,SA1.A1_INSCR,"
				cQuery += "SA2.A2_INSCR,SA2.A2_TIPO,SA2.A2_TIPORUR,SA2.A2_MUN"
			EndIF
			...
			cIPI := "select D1_CF,SUM(SD1.D1_VALIPI) AS D1_VALIPI "
			cIPI += "FROM " 
			cIPI += RetSqlName("SD1") + " SD1 "	 
			cIPI += "JOIN "+RetSqlName("SF4")+" SF4 ON F4_FILIAL ='"+xFilial("SF4")+"' AND D1_TES = F4_CODIGO "
			cIPI += "WHERE SD1.D1_FILIAL = '"+xFilial("SD1")+"' AND  F4_INCIDE = 'N' AND  F4_LFIPI <> 'P' AND "		
			If nGeraTransp == 1 .And. lF3TRFICM 
				cIPI += " ((SD1.D1_EMISSAO >= '"+DTOS(dDtIni)+"' AND SD1.D1_EMISSAO <= '"+DTOS(dDtFim)+"'"																		
				cIPI += " AND SD1.D1_CF NOT IN ('1601','1602','1605','5601','5602','5605') )
		   		If SuperGetMv('MV_ESTADO')=='PR'
					cIPI += " OR (SD1.D1_EMISSAO >= '"+DTOS(dDtIni+5)+"' AND SD1.D1_EMISSAO <= '"+DTOS(dDtFim+5)+"'"
	   			Else		   		
					cIPI += " OR (SD1.D1_EMISSAO >= '"+DTOS(dDtIni+9)+"' AND SD1.D1_EMISSAO <= '"+DTOS(dDtFim+9)+"'"
		   		EndIf
				cIPI += " AND SD1.D1_CF IN ('1601','1602','1605','5601','5602','5605') )) AND "			
			Else
				cIPI += "SD1.D1_EMISSAO >= '"+DTOS(dDtIni)+"' AND "		
				cIPI += "SD1.D1_EMISSAO <= '"+DTOS(dDtFim)+"' AND "
				cIPI += "SD1.D1_CF NOT IN ('1601','1602','1605','5601','5602','5605') AND "
			Endif
			If cNrLivro <> "*"
					cIPI += "F4_NRLIVRO ='"+ cNrLivro +"' AND "
			EndIf
			If lCodRSEF
				cIPI += "EXISTS(SELECT F3_NFISCAL FROM " +RetSqlName("SF3") +" SF3 WHERE " 
				cIPI += "SF3.F3_FILIAL= '"+xFilial("SF3")+"'  And " 
				cIPI += "SF3.F3_NFISCAL=SD1.D1_DOC And " 
				cIPI += "SF3.F3_SERIE=SD1.D1_SERIE And "
				cIPI += "SF3.F3_CLIEFOR=SD1.D1_FORNECE And " 
				cIPI += "SF3.F3_LOJA=SD1.D1_LOJA AND " 
				cIPI += "SF3.F3_CFO=SD1.D1_CF AND SF3.F3_CODRSEF NOT IN('102','110','204','205','206','301','302','303','304','305','306')) AND "
			Endif
			cIPI += "SD1.D_E_L_E_T_ = ' '"
			cIPI += "GROUP BY D1_CF "
			cIPI += "UNION ALL "
			cIPI += "select D2_CF,SUM(SD2.D2_VALIPI) AS D2_VALIPI "
			cIPI += "FROM "+ RetSqlName("SD2") + " SD2 "	 
			cIPI += "JOIN "+RetSqlName("SF4")+" SF4 ON F4_FILIAL ='"+xFilial("SF4")+"' AND D2_TES = F4_CODIGO "
			cIPI += "WHERE SD2.D2_FILIAL = '"+xFilial("SD2")+"' AND F4_INCIDE = 'N' AND F4_LFIPI <> 'P' AND "		
			If nGeraTransp == 1 .And. lF3TRFICM 
				cIPI += " ((SD2.D2_EMISSAO >= '"+DTOS(dDtIni)+"' AND SD2.D2_EMISSAO <= '"+DTOS(dDtFim)+"'"																		
				cIPI += " AND SD2.D2_CF NOT IN ('1601','1602','1605','5601','5602','5605') )
		   		If SuperGetMv('MV_ESTADO')=='PR'
					cIPI += " OR (SD2.D2_EMISSAO >= '"+DTOS(dDtIni+5)+"' AND SD2.D2_EMISSAO <= '"+DTOS(dDtFim+5)+"'"
	   			Else		   		
					cIPI += " OR (SD2.D2_EMISSAO >= '"+DTOS(dDtIni+9)+"' AND SD2.D2_EMISSAO <= '"+DTOS(dDtFim+9)+"'"
		   		EndIf
					cIPI += " AND SD2.D2_CF IN ('1601','1602','1605','5601','5602','5605') )) AND "			
			Else
					cIPI += "SD2.D2_EMISSAO >= '"+DTOS(dDtIni)+"' AND "		
					cIPI += "SD2.D2_EMISSAO <= '"+DTOS(dDtFim)+"' AND "
					cIPI += "SD2.D2_CF NOT IN ('1601','1602','1605','5601','5602','5605') AND "
			Endif		
			If cNrLivro <> "*"
					cIPI += "F4_NRLIVRO ='"+ cNrLivro +"' AND "
			EndIf
			If lCodRSEF
				cIPI += "EXISTS(SELECT F3_NFISCAL FROM " +RetSqlName("SF3") +" SF3 WHERE " 
				cIPI += "SF3.F3_FILIAL= '"+xFilial("SF3")+"'  And " 
				cIPI += "SF3.F3_NFISCAL=SD2.D2_DOC And " 
				cIPI += "SF3.F3_SERIE=SD2.D2_SERIE And "
				cIPI += "SF3.F3_CLIEFOR=SD2.D2_CLIENTE And " 
				cIPI += "SF3.F3_LOJA=SD2.D2_LOJA AND " 
				cIPI += "SF3.F3_CFO=SD2.D2_CF AND SF3.F3_CODRSEF NOT IN('102','110','204','205','206','301','302','303','304','305','306')) AND "
			Endif
			cIPI += "SD2.D_E_L_E_T_ = ' '"
			cIPI += "GROUP BY D2_CF "
			cIPI += "ORDER BY D1_CF "
			
			cQuery := ChangeQuery(cIPI)
		   	dbUseArea(.T.,"TOPCONN",TcGenQry(,,cIPI),cAliasIpi,.T.,.T.)
		    while (cAliasIpi)->(!Eof())
		   		AADD(aAliasIpi,{(cAliasIpi)->D1_CF,(cAliasIpi)->D1_VALIPI})
		   		(cAliasIpi)->(DBSKIP())	   
		    Enddo	
		   	(cAliasIpi)->(dbclosearea())
	...
	While (cAliasSF3)->(!Eof()) .and. xFilial("SF3")==(cAliasSF3)->F3_FILIAL
		...
		*/
		...
		If ((cAliasSF3)->F3_TIPO=="S" ) .OR. (cCfop3 $"000#999") .Or. (cCfop4 $"1601#1602#1605#5601#5602#5605#5927")
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				//³Grava arquivo temporario CR=25 (cArqIE)³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				If !lRelDIPAM .And. (cCfop3 $"000#999" .Or. cCfop4 $"1601#1602#1605#5601#5602#5605#5927")
					IF lF3TRFICM
						a972TmpIE(cAliasSF3)
					EndIF
		       Endif
		...
		EndIf
                ...
                Else //(cRef>"200804")
					...
					ElseIf (cCfop1$"12")
						If (((nValCFOP<>1360) .And. (nValCFOP>=1401) .And. (nValCFOP<=1449)) .Or.;
							...
							((nValCFOP>=2401) .And. (nValCFOP<=2449)) .Or.;
							((nValCFOP>=2651) .And. (nValCFOP<=2699)) .Or.;
							((nValCFOP>=1949) .And. (nValCFOP<=2949)) .And. (nValCFOP<>1603) .And. (nValCFOP<>2603).Or.;
							(cCfop2=="19") .Or.; 
							(cCfop2=="29"))			
							If (cAliasSF3)->F3_VALANTI == 0				
								If	(cAliasSF3)->F3_CREDST $ "2 "								
									(cArqCFO)->RtStSt += (cAliasSF3)->F3_ICMSRET
									lStSb		:=	.T.
		   		...
		   		EndIf
            Endif
            // O F3_IPIOBS é sempe parte da base de icms e do total da NF, mas é gravado no livro separadamente, por isso, sempr soma-se em outros impostos
            If Empty((cArqCFO)->IMPOSTO)
				 nValIpi:=0
				 aeval(aAliasIpi,{|x| IIF(AllTrim(x[1])==cCfop4,nValIpi += x[2],0)})
            	 (cArqCFO)->IMPOSTO	+=	nValIpi 
			  Endif 
			...
			//Somente irá processar este trecho para considerar PIS e COFINS no campo (cArqCFO)->IMPOSTO somente se for ambiente DBF.
				...
				Do Case
					Case (cVLayOut=="0208" .And. Alltrim(SM0->M0_CNAE)=='5620101') .Or. ; //tratamento para CNAE de Refeicoes fora do Municipio
						...
						If ( (cAliasSF3)->F3_BASEICM + (cAliasSF3)->F3_ISENICM + (cAliasSF3)->F3_OUTRICM > 0 )
							...
							Endif
						EndIf
					Case ( (nAno<2003 .And. cCfop3$"561/562/563/661/662/663/761") .Or. (nAno>=2003 .And. cCfop4$"5351/5352/5353/5354/5355/5356/5357/5358/5359/5360/6351/6352/6353/6354/6355/6356/6357/6358/6359/6360/7358" .Or. cCfop4$"1351/2351/3355"))
						If ( (cAliasSF3)->F3_BASEICM + (cAliasSF3)->F3_ISENICM + (cAliasSF3)->F3_OUTRICM > 0 )
							dbSelectArea(cArqDipam)
							dbSetOrder(1)
							n23Ent := 0
							...
							If !lRelDIPAM
								If cCfop4$"1351/2351/3355"                         
								   n23Ent += (cArqCFO)->BASEICM + (cArqCFO)->ISENTA + (cArqCFO)->OUTRAS + (cArqCFO)->IMPOSTO                                 
								Else
									n23Sai += (cArqCFO)->BASEICM + (cArqCFO)->ISENTA + (cArqCFO)->OUTRAS + (cArqCFO)->IMPOSTO
								Endif
								...
								Else
									RecLock(cArqDipam,.F.)
								Endif
								if (nPos := ascan(aDipamB23,{|X| X[1]=='23' .and. X[2]==(cArqDipam)->MUNICIP} )) == 0
									aadd(aDipamB23,{'23',(cArqDipam)->MUNICIP,n23sai-n23ent })
									npos := len(aDipamB23)
								else
									aDipamB23[npos][3] += n23sai-n23ent
								Endif	
								VALOR := abs(aDipamB23[npos][3]) 
								MsUnLock()
							Else
					...
					Case ((nAno>=2003 .And. (cCfop4$"5301/5302/5303/5304/5305/5306/5307/6301/6302/6303/6304/6305/6306/6307/7301") .OR. cCfop4$"1301/2301/3301"))
							...
							If !lRelDIPAM
								...
								Else
									...
									MUNICIP:= cCodMunDp
								   	(cArqDipam)->TIPOREG	:=  StrZero(nTipoReg,6) 
									if (nPos := ascan(aDipamB23,{|X| X[1]=='24' .and. X[2]==(cArqDipam)->MUNICIP} )) == 0
										aadd(aDipamB23,{'24',(cArqDipam)->MUNICIP,n23sai-n23ent })
										npos := len(aDipamB23)
									else
										aDipamB23[npos][3] += n23sai-n23ent
									Endif	           
	...
	If lTop .AND. !lRelDIPAM	
		...
		IF lGeraExp			
		    ...
		    Elseif  !lD2_Re
				...
				Do While !(cAliasF3)->(EOF())			
					If EE9->(msseek(xFilial("EE9")+(cAliasF3)->D2_PREEMB))				
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						//³Processamento da funcao da Average que retorna os avisos de embarque conforme regras do SIGAEEC ³
						//|Conforme solicitado nos Chamados: THMQEB e THMOIB, he necessário as datas de RE e DSE para o R31|
						//|                     FUNCAO DISPONIBILIZADA PELA AVEGARE - FONTE AVGERAL.PRW                    |
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ						
						acRe := EasySpedRes(cValToChar(Month((cAliasF3)->D2_EMISSAO)),cValToChar(Year((cAliasF3)->D2_EMISSAO)))
...
/*			If nX == 1
				//Saídas
				cSlctSFT	:= "%SUM(SD2.D2_VALIMP6) as VALIMP6 , SUM(SD2.D2_VALIMP5) as VALIMP5, SF3.F3_CFO%"
				cJoinSF4	:= "%SF4.F4_CODIGO=SD2.D2_TES%"
				cJoinD1D2	:=	"%JOIN "+RetSqlName("SD2")+" SD2 ON(SD2.D2_FILIAL='"+xFilial("SD2")+"' AND SD2.D2_DOC=SF3.F3_NFISCAL AND SD2.D2_SERIE=SF3.F3_SERIE AND SD2.D2_CLIENTE=SF3.F3_CLIEFOR AND SD2.D2_LOJA=SF3.F3_LOJA AND SD2.D_E_L_E_T_=' ') %"
			Else
				//Entradas
				cSlctSFT	:= "%SUM(SD1.D1_VALIMP6) as VALIMP6 , SUM(SD1.D1_VALIMP5) as VALIMP5, SF3.F3_CFO%"
				cJoinSF4	:= "%SF4.F4_CODIGO=SD1.D1_TES%"
				cJoinD1D2	:=	"%JOIN "+RetSqlName("SD1")+" SD1 ON(SD1.D1_FILIAL='"+xFilial("SD1")+"'  AND SD1.D1_DOC=SF3.F3_NFISCAL AND SD1.D1_SERIE=SF3.F3_SERIE AND SD1.D1_FORNECE=SF3.F3_CLIEFOR AND SD1.D1_LOJA=SF3.F3_LOJA AND SD1.D_E_L_E_T_=' ') %"				
			EndIF			
			...
			EndIf
			cFiltro += "SF3.F3_DTCANC =' ' AND "
			cFiltro += "SF4.F4_AGRPIS IN ('1','S') AND SF4.F4_AGRCOF IN('1','S') AND %"
			
			cGroupBy	:= "% GROUP BY SF3.F3_CFO%
			...
			BeginSql Alias cAliasF3		    	
				...
				%Table:SF3% SF3
				%Exp:cJoinD1D2%
				JOIN %Table:SF4% SF4 ON(SF4.F4_FILIAL=%xFilial:SF4% AND %Exp:cJoinSF4% AND SF4.%NotDel%)				
			...
			If nX == 1
				//Saídas
				cSlctSFT	:= "%SFT.FT_CFOP, " 
				cSlctSFT	+= "(SELECT SUM(SD21.D2_VALIMP6) VALIMP6 FROM "+RetSqlName("SD2")+" SD21 "
				cSlctSFT	+= "JOIN "+RetSqlName("SF4")+" SF41 ON SF41.F4_CODIGO = SD21.D2_TES AND SF41.D_E_L_E_T_ = ' ' AND SF41.F4_FILIAL = '"+xFilial("SF4")+"' "
				cSlctSFT	+= "WHERE SD21.D2_CF = SFT.FT_CFOP AND SF41.F4_AGRPIS IN ('1', 'S') AND SD21.D_E_L_E_T_ = ' ' AND SD21.D2_FILIAL = '"+xFilial("SD2")+"' AND   SD21.D2_EMISSAO >= '"+DTOS(dDtIni)+"' AND SD21.D2_EMISSAO <= '"+DTOS(dDtFim)+"' ) VALIMP6, "

				cSlctSFT	+= "(SELECT SUM(SD22.D2_VALIMP5) VALIMP5 FROM "+RetSqlName("SD2")+" SD22 "
				cSlctSFT	+= "JOIN "+RetSqlName("SF4")+" SF42 ON SF42.F4_CODIGO = SD22.D2_TES AND SF42.D_E_L_E_T_ = ' ' AND SF42.F4_FILIAL = '"+xFilial("SF4")+"' "
				cSlctSFT	+= "WHERE SD22.D2_CF = SFT.FT_CFOP AND SF42.F4_AGRCOF IN ('1', 'S') AND SD22.D_E_L_E_T_ = ' ' AND SD22.D2_FILIAL = '"+xFilial("SD2")+"' AND   SD22.D2_EMISSAO >= '"+DTOS(dDtIni)+"' AND SD22.D2_EMISSAO <= '"+DTOS(dDtFim)+"' ) VALIMP5 %"
			Else
				//Entradas
				cSlctSFT	:= "%SFT.FT_CFOP, " 
				cSlctSFT	+= "(SELECT SUM(SD11.D1_VALIMP6) VALIMP6 FROM "+RetSqlName("SD1")+" SD11 "
				cSlctSFT	+= "JOIN "+RetSqlName("SF4")+" SF41 ON SF41.F4_CODIGO = SD11.D1_TES AND SF41.D_E_L_E_T_ = ' ' AND SF41.F4_FILIAL = '"+xFilial("SF4")+"' "
				cSlctSFT	+= "WHERE SD11.D1_CF = SFT.FT_CFOP AND SF41.F4_AGRPIS IN ('1', 'S') AND SD11.D_E_L_E_T_ = ' ' AND SD11.D1_FILIAL = '"+xFilial("SD1")+"' AND SD11.D1_DTDIGIT >= '"+DTOS(dDtIni)+"' AND SD11.D1_DTDIGIT <= '"+DTOS(dDtFim)+"' ) VALIMP6, "

				cSlctSFT	+= "(SELECT SUM(SD12.D1_VALIMP5) VALIMP5 FROM "+RetSqlName("SD1")+" SD12 "
				cSlctSFT	+= "JOIN "+RetSqlName("SF4")+" SF42 ON SF42.F4_CODIGO = SD12.D1_TES AND SF42.D_E_L_E_T_ = ' ' AND SF42.F4_FILIAL = '"+xFilial("SF4")+"' "
				cSlctSFT	+= "WHERE SD12.D1_CF = SFT.FT_CFOP AND SF42.F4_AGRCOF IN ('1', 'S') AND SD12.D_E_L_E_T_ = ' ' AND SD12.D1_FILIAL = '"+xFilial("SD1")+"' AND SD12.D1_DTDIGIT >= '"+DTOS(dDtIni)+"' AND SD12.D1_DTDIGIT <= '"+DTOS(dDtFim)+"' ) VALIMP5 %"
			EndIF
			...
			Do While !(cAliasFT)->(EOF())
				...
				If (cArqCFO)->(msseek((cAliasFT)->FT_CFOP+StrZero(nTipoReg,6)))
					RecLock(cArqCFO,.F.)
					(cArqCFO)->IMPOSTO += (cAliasFT)->VALIMP5 + (cAliasFT)->VALIMP6 
					MSUnLock()
				EndIF
	...
	If (cArqDipam)->(msseek("24"))
		While !((cArqDipam)->(Eof()))
			If (cArqDipam)->CODDIP == "24"
				...
				RecLock(cArqDipam,.F.)
				If ExistBlock("MA972VLR") //Força a gravação do valor simbólico de R$ 1,00 
					nValor := ExecBlock("MA972VLR",.F.,.F.,{cArqDipam})
					If ValType(nValor)<>"N"
						nValor := 0
					Endif	    
				Else
...
If !lRelDIPAM .And. n23Ent > 0
	//tratamento para o código 23 do CR=30
	n23Tot := n23Sai - n23Ent
	dbSelectArea(cArqDipam)
	If (cArqDipam)->(msseek("23"))
		While !((cArqDipam)->(Eof()))
			If (cArqDipam)->CODDIP == "23"
				nPerc := (cArqDipam)->VALOR/n23Sai
				RecLock(cArqDipam,.F.)
				If ExistBlock("MA972VLR") //Força a gravação do valor simbólico de R$ 1,00 
					nValor := ExecBlock("MA972VLR",.F.,.F.,{cArqDipam})
					If ValType(nValor)<>"N"
	...
	While (cArqOcor)->(!eof())		
		...
		If cRef > "200112"
			...
			ElseIf alltrim((cArqOcor)->SUBITEM)$ "|00219|00730|"
				...
				nRemetent := 0
				While !(cArqIE)->(Eof())
					IF (alltrim((cArqOcor)->SUBITEM)$'|00219|' .AND. SUBSTR((cArqIE)->SUBITEM,1,1) $ '5') .OR. (alltrim((cArqOcor)->SUBITEM)$'|00730|' .AND. SUBSTR((cArqIE)->SUBITEM,1,1) $ '1')
		...
		a972Fill(aOcorr[n20][5] ,100)+a972Fill(aOcorr[n20][6] ,300)+a972Fill(Num2Chr(iif(nRemetent > 0,nRemetent,Len(aRemetent)),4,0),04)  +;
