...
\====================================================================================*/
*-----------------------------*
User Function STOV2019()   //  u_STOV2017()
	*-----------------------------*
	//-- Dimensoes padroes
	...
	*/
	...
	DEFINE MSDIALOG oDlg TITLE cCadastro FROM 0,0 TO aSize[6]+500,aSize[5]-30   of GetWndDefault() Pixel
	...
	*/
	...
	_nlin+= _nlin7
	@ _nlin,005  ComboBox oStat01a VAR cStat01a ITEMS {' ','1-Homem','2-Mulher','3-Prefere não responder','4-Auto declara-se como:'} Of oDlg  PIXEL SIZE 85,10 //Valid lCont:= VerCom(cStat01a)

	oGet1:= TGet():New( _nlin, 095, { | u | If( PCount() == 0, cGet1, cGet1 := u ) },oDlg, 150, 008, "!@",, 0, 16777215,,.F.,,.t.,,.F., {|| VldComo(cStat01a)}  ,.F.,.F.,,.F.,.F. ,,"cGet1",,,,.F.  )
	...
	*/
	...
	@ _nlin,005 	Say "19. Descreva um momento que fez você sentir orgulho em trabalhar na Steck:" PIXEL of oDlg  FONT oFontN
	_nlin+= _nlin7
	 oDesc := tMultiget():new( _nlin, 005, {| u | if( pCount() > 0, cDesc := u, cDesc ) },  oDlg, 460, 30, , , , , , .T. )
	 _nlin+= _nlin12
	//_nlin+= _nlin7
	@ 001,001  To _nlin+20,660   PIXEL of oDlg
	//_nlin+= _nlin12
...
Static Function STGRVMIX()
	...
	PPC->PPC_01 	:= cStat01a //IIf(lChk2,'F','M')//2018
...
Static Function STFINATOT()
	*---------------------------------------------------*
	...
	'TI',;
	'Vendas',;
	'Expedição/Transporte/Logística/Comex',;
...
Static Function VldComo(cStat01a)

	If SubStr(cStat01a,1,1) $ "4"
		lCont:= .T.
	Else
		lCont:= .F.
	EndIf
