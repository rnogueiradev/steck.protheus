...
Method SetOrigem(cOrigem) Class STzExcelXML
	...
	If SubStr(cOrigem, Len(Alltrim(cOrigem))-3, Len(Alltrim(cOrigem))) != ".xml"
		lRet := .F.
		cMensagem :=	"[STzExcelXML][004] Arquivo de ORIGEM com extensão inválida! "+;
	...
	ElseIf File(cOrigem)
		...
		Else
			lRet := .F.
			cMensagem :=	"[STzExcelXML][002] Arquivo de ORIGEM inválido! "+;
							"Verificar! - SetOrigem('"+cOrigem+"') - "+dToC(dDataBase)+" "+Time()
		EndIf
	...
	Else
		lRet := .F.
		cMensagem :=	"[STzExcelXML][001] Arquivo de ORIGEM não foi encontrado! "+;
						"Verificar! - SetOrigem('"+cOrigem+"') - "+dToC(dDataBase)+" "+Time()
	EndIf
...
Method SetDestino(cDestino, lSobrepoe) Class STzExcelXML
	...
	If SubStr(cDestino, Len(Alltrim(cDestino))-3, Len(Alltrim(cDestino))) != ".xml"
		lRet := .F.
		cMensagem :=	"[STzExcelXML][005] Arquivo de DESTINO com extensão inválida! "+;
	...
	ElseIf !Empty(cDestino)
		...
		If File(cDestino)
			cMensagem :=	"[STzExcelXML][003] Arquivo de Destino já existe! "+;
							"Verificar! - SetDestino('"+cDestino+"') - "+dToC(dDataBase)+" "+Time()
...
Method AddExpression(cExpressao, xConteudo) Class STzExcelXML
	...
	//Senão
	Else
		cMensagem :=	"[STzExcelXML][013] Expressão em branco ou inválida não pode ser adicionada! "+;
...
Method AddTabExcel(cNomeTab, cAliasTab) Class STzExcelXML
	...
	//Senão
	Else
		cMensagem :=	"[STzExcelXML][014] Tabela em branco não pode ser adicionada! "+;
...
Method CopyTo(cCopia) Class STzExcelXML
	...
	If Empty(cCopia)
		cMensagem :=	"[STzExcelXML][011] Caminho do arquivo em cópia está em branco! "+;
...
Method View() Class STzExcelXML
	...
	Else
		cMensagem :=	"[STzExcelXML][010] Arquivo não pode ser aberto, utilize o método MountFile() para montar os dados! "+;
						"Verificar! - View() - "+dToC(dDataBase)+" "+Time()
	EndIf
...
Method ViewSO() Class STzExcelXML
	...
	If Len(::aConDest) > 0
		...
		If nRet <= 32
			cMensagem :=	"[STzExcelXML][008] Arquivo não pode ser aberto! "+;
							"Verificar! - ViewSO() - "+dToC(dDataBase)+" "+Time()
		EndIf 
		
	//Senão, mostra mensagem de erro
	Else
		cMensagem :=	"[STzExcelXML][009] Arquivo não pode ser aberto, utilize o método MountFile() para montar os dados! "+;
						"Verificar! - ViewSO() - "+dToC(dDataBase)+" "+Time()
	EndIf
...
Method Destroy(lShowAlerts) Class STzExcelXML
	Local cMensagem := "[STzExcelXML][999] Objeto Destruído com sucesso!"
	Default lShowAlerts := .F.
