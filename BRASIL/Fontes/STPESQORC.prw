...
|=====================================================================================|
|Descrição |  STPESQORC    Pesquisa ORÇAMENTO	                                      |
|          |                                                                          |
|          |                                                                          |
|=====================================================================================|
|Sintaxe   | STPESQORC                                                                |
...
User Function STPESQORC()   //  u_STPESQORC()
	*-----------------------------*
	...
	//EndIf
	Private cCadastro := "Pesquisa Orçamentos"
	Private oDlg
	Private oGetDados1
	Private oGetDados2
	Private oGetDados3
	...
	Define msDialog _oDxlg Title "Pedido/Orçamento" From 10,10 TO 100,120 Pixel
	
	@ 005,005 Button "&Pedido"   size 40,14  action ((lSaida:=.T.,_nOpcao:=2,_oDxlg:End())) Of _oDxlg Pixel
	
	
	@ 021,005 Button "&Orçamento" size 40,14  action ((lSaida:=.T.,_nOpcao:=1,_oDxlg:End())) Of _oDxlg Pixel
	...
	@ aPosObj[1,3]/2-16+_nPosTel,006  SAY 'Orçamentos'	PIXEL FONT oFontN OF  oDlg
	...
	If Len(oGetDados1:acols) > 0
		...
		If !Empty(Alltrim(_cAdd))
			...
			If  Select(cAliasLif) > 0
				...
				While !(cAliasLif)->(Eof())
					IncProc("Lendo Orçamento: "+(cAliasLif)->COD)
	...
	If  Select(cAliasLif) > 0
		...
		While !(cAliasLif)->(Eof())
			...
			If SB1->(DbSeek(xFilial("SB1")+	(cAliasLif)->UB_PRODUTO))
				...
				(cAliasLif)->UB_QUANT	 ,;	//04   descrição
				(cAliasLif)->UB_VRUNIT	 ,;
					.f.	})
				_cSoma:=	Soma1(_cSoma)
			EndIf
