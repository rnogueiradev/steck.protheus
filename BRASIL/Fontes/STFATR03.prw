...
User Function STFATR03()
	...
	If !( __cUserId $ Getmv("ST_FATR03",,"000000/000645/000294/000965/000271/000231/000391/000591"))
		...
		MsgInfo("Usuario sem acesso, solicite liberaÁ„o ao Vanderlei ...!!!")
		Return()
...
Static Function ReportDef()
	...
	oReport := TReport():New("STFATR03","Captacao Anual","STFATR03",{|oReport| ReportPrint(oReport)},"Este programa ir· imprimir a captaÁ„o anual conforme parametros selecionados.")
...
Static Function ReportPrint(oReport)
	...
	If Len(aEmpresa) > 0
    	...
    	oTable:AddIndex("02", {"TRB_TIP", "TRB_AGR", "TRB_GRP", "TRB_COD" } )
		...
		For _i := 1 to Len(aEmpQry)
			...
			If aEmpQry[_i] = '11' //Distribuidora
				...
				cQuery += " AND (SC6.C6_BLQ <> 'R'  OR  SC6.C6_BLQ = 'R' AND SC6.C6_QTDENT > 0)
				IF aEmpQry[_i]="11"
					cQuery += " AND PC1_PEDREP IS NULL AND"+ CRLF
			...
			ElseIf aEmpQry[_i] = '03' //Distribuidora
			...
			ElseIf aEmpQry[_i] = '07'
				...
				cQuery += " FROM UDBP12ARG.SC5"+aEmpQry[_i]+"0 C5 "
				cQuery += " INNER JOIN(SELECT * FROM UDBP12ARG.SC6"+aEmpQry[_i]+"0) C6  ON ( C6_FILIAL = C5_FILIAL  AND C6_NUM = C5_NUM  AND C6.D_E_L_E_T_ = ' ' )
				cQuery += " LEFT    JOIN UDBP12ARG.SM2"+aEmpQry[_i]+"0 M2  ON ( M2_DATA = C5_EMISSAO   AND M2.D_E_L_E_T_ = ' ' )"
				cQuery += " INNER   JOIN UDBP12ARG.SF4"+aEmpQry[_i]+"0 SF4 ON ( F4_CODIGO = C6_TES     AND F4_TIPO = 'S' AND F4_DUPLIC = 'S' AND F4_ESTOQUE = 'S' AND SF4.D_E_L_E_T_ = ' ' )"
				cQuery += " LEFT    JOIN UDBP12ARG.SFB"+aEmpQry[_i]+"0 FB  ON ( FB.FB_FILIAL = ' '     AND FB_CODIGO = 'IVA' AND FB.D_E_L_E_T_ = ' ' )"
				cQuery += " INNER   JOIN UDBP12ARG.SA1"+aEmpQry[_i]+"0 A1  ON ( A1_COD = C5_CLIENTE    AND A1_LOJA = C5_LOJACLI AND A1_GRPVEN <> 'ST' AND A1.D_E_L_E_T_ = ' ' )"
			...
			ElseIf aEmpQry[_i] = '09'

				cQuery += " SELECT '09' EMP,'6' TIP,SUBSTRING(C5_EMISSAO,5,2)DAT,C6_PRODUTO COD,C5_CLIENTE CLI,C5_LOJACLI LOJ, SUM(C6_VALOR) C6_VALOR "
				cQuery += " FROM UDBCOL.SC6090 SC6 "
				cQuery += " INNER JOIN(SELECT * FROM UDBCOL.SC5090) SC5
		...
		cQuery += " LEFT JOIN (SELECT '11' EMP, BM_XAGRUP,BM_GRUPO,D_E_L_E_T_ FROM SBM110 WHERE D_E_L_E_T_ = ' ' UNION SELECT '03' EMP,BM_XAGRUP,BM_GRUPO,D_E_L_E_T_ FROM UDBP12.SBM030 WHERE D_E_L_E_T_ = ' ' UNION SELECT '07' EMP,BM_XAGRUP,BM_GRUPO,D_E_L_E_T_ FROM UDBP12ARG.SBM070 WHERE D_E_L_E_T_ = ' ' UNION SELECT '09' EMP,BM_XAGRUP,BM_GRUPO,D_E_L_E_T_ FROM UDBCOL.SBM090 WHERE D_E_L_E_T_ = ' '  ) BM ON B1.B1_GRUPO = BM.BM_GRUPO AND BM.EMP = B1.EMP AND BM.D_E_L_E_T_ = ' '
