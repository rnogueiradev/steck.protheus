...
User Function STEJOB01(cCdEmp,cCdFil)
	...
	For ni := 1 to Len(aLstEmp)
		...
		If  oFunc:SetRPCCon(cEmpCon,cFilCon," ")
				...
				If lOk
					...
					Do While !(cAliasT)->(Eof())
						...
						If !SE1->(Eof())
							...
							If DaySub(SE1->E1_VENCREA,nDias) <= MsDate() //giovani zago ajuste ticket 20200406001466
					...
					BeginSQL Alias cAliasT
						...
						WHERE SE1.%notDel%
						AND SE1.E1_FILIAL = %xFilial:SE1%
						AND SE1.E1_VENCREA < %exp:MsDate()% //giovani zago ajuste ticket 20200406001466
						AND SE1.E1_TIPO = %exp:"NF"%
						AND SE1.E1_SALDO > 0
						AND EXISTS (
						SELECT 1
					...
					For nx := 1 to Len(aLxVencer)
						...
						//Mensagem
						cMens := "Prezado Cliente: " + Eval(bForma01,cNomeCli) + Replicate(CRLF,2)
						cMens += "Para sua maior comodidade informamos abaixo a relação dos títulos com vencimentos nesta semana." + Replicate(CRLF,2)
						cMens += "Favor confirmar os dados das notas fiscais, indicados abaixo." + Replicate(CRLF,2)
						cMens += "Caso haja divergência com sua documentação, "
						cMens += "ou caso esta mensagem tenha sido enviada para o departamento/setor não compatível, "
						cMens += "pedimos a gentileza de entrar em contato com eduardo.santos@steck.com.br / gleice.freitas@steck.com.br, " //tICKET 20200929008103
						cMens += "ou via telefone (11) 2248-7072."+ Replicate(CRLF,2)
						cMens += "Atenciosamente," + Replicate(CRLF,2)
						cMens += "Departamento Financeiro" + Replicate(CRLF,2)
					...
					For nx := 1 to Len(aLxAtraso)
						...
						//Mensagem
						cMens := "Prezado Cliente: " + Eval(bForma01,cNomeCli) + Replicate(CRLF,2)
						cMens += "Para sua maior comodidade informamos abaixo a relação dos títulos vencidos, até a presente data." + Replicate(CRLF,2)
						cMens += "Caso haja divergência com sua documentação, "
						cMens += "ou caso esta mensagem tenha sido enviada para o departamento/setor não compatível, "
						cMens += "pedimos a gentileza de entrar em contato com eduardo.santos@steck.com.br / gleice.freitas@steck.com.br, "
						cMens += "ou via telefone (11) 2248-7072."+ Replicate(CRLF,2)
						cMens += "Favor desconsiderar este aviso caso já tenha regularizado os pagamentos." + Replicate(CRLF,2)
						cMens += "Atenciosamente," + Replicate(CRLF,2)
						cMens += "Departamento Financeiro" + Replicate(CRLF,2)
						//Varrer todos os titulos do cliente
						aMensIt := {}
			...
			EndIf
		Else
			//Eval(bMens,"Nao foi possivel estabelecer a conexao RPC [" + cEmpCon + "-" + cFilCon + "].",ERR_RECEIV)
		Endif
	Next ni
...
Static Function Scheddef()
	...
	PutSx1("STEJOB01", "01","Empresa"	,"Empresa"	,"Empresa"	,"mv_ch1","C",2,0,0,"G","",''    ,"","","mv_par01","","","","","","","","","","","","","","","","")
	PutSx1("STEJOB01", "02","Filial"	,"Filial"	,"Filial"  	,"mv_ch2","C",2,0,0,"G","",''    ,"","","mv_par02","","","","","","","","","","","","","","","","")
	
	aParam := {"P","STEJOB01","SE1",aOrd,"Emp_Mail"}
