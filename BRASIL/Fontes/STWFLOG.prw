...
#ELSE
	...
	*ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	@ 11,12 LISTBOX oLbx FIELDS HEADER OemToAnsi("Arq    "),OemToAnsi("Descrição")  SIZE 131, 69 OF oDlg PIXEL ; // "Arq    " ###  "Descri‡„o"
...
#ELSE
	...
	DEFINE MSDIALOG oDlg FROM  16,2 TO 296,475 TITLE OemToAnsi("Log do Registro") PIXEL // "Log do Registro"
			...
			@ 7, 124 TO 57, 229 LABEL OemToAnsi("Log de Inclusão") OF oDlg  PIXEL // "Log de Inclus„o"
			@ 65, 124 TO 115, 229 LABEL OemToAnsi("Log de Alteração") OF oDlg  PIXEL // "Log de Altera‡„o"
			@ 8,8 LISTBOX oLbx FIELDS HEADER OemToAnsi("Campos"),OemToAnsi("Conte£do")  SIZE 110, 68 OF oDlg PIXEL  // "Campos" ### "Conte£do"
			   ...
			   oLbx:bLine := { || {aCampos[oLbx:nAt,1],aCampos[oLbx:nAt,2]}}
				...
				@ 22, 134 SAY OemToAnsi("Usuário:") SIZE 26, 7 OF oDlg PIXEL // "Usu rio:"
				...
				@ 79, 134 SAY OemToAnsi("Usuário:") SIZE 26, 7 OF oDlg PIXEL // "Usu rio:"
...
Else
	cUsuarioI := OemtoAnsi("Log Não Ativo") //"Log N„o Ativo"
Endif
If cTipo $ "23"
...
Else
	cUsuarioA := OemtoAnsi("Log Não Ativo") // "Log N„o Ativo"
Endif
Return
...
Local cDesc1 := "Este programa irá  imprimir a Consulta ao LOG de"
...
If MV_PAR01 == 2
	...
	While !Eof()
		...
		u_LeLog(@cStatus,@cUsuarioI,@cUsuarioA,@cDataI,@cDataA,cTipo)
		// converte as informações para o formato Data
		cDataI := Ctod(cDataI)
		cDataA := Ctod(cDataA)
		If !Empty(MV_PAR02)
