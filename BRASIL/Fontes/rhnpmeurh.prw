...
//geração do html para o recibo de férias
	If lRet
		...
		nLin += 50
		cMsgLine := AllTrim(aDados[2]) +', ' +SubStr(DtoC( aDados[3] ),1,2) +' de ' +MesExtenso(Month(aDados[3])) +' de ' +STR(Year(aDados[3]),4)
...
User Function VacationReport( aCabec, aVerbas, aInfo, cLocal, cFileName )
	...
	If !Empty(aCabec) .And. !Empty(aVerbas) .And. !Empty(aInfo)
		...
		PER_AQ_I := STRZERO( DAY( aCabec[1,08]),2) + ' de ' + MesExtenso(MONTH( aCabec[1,08])) + ' de ' + STR(YEAR( aCabec[1,08]),4)	//" De "###" De "
		PER_AQ_F := STRZERO( DAY( aCabec[1,09]),2) + ' de ' + MesExtenso(MONTH( aCabec[1,09])) + ' de ' + STR(YEAR( aCabec[1,09]),4)	//" De "###" De "
		PER_GO_I := STRZERO( DAY( aCabec[1,04]),2) + ' de ' + MesExtenso(MONTH( aCabec[1,04])) + ' de ' + STR(YEAR( aCabec[1,04]),4)	//" De "###" De "
		PER_GO_F := STRZERO( DAY( aCabec[1,05]),2) + ' de ' + MesExtenso(MONTH( aCabec[1,05])) + ' de ' + STR(YEAR( aCabec[1,05]),4)	//" De "###" De "
		...
		//Data de pagamento
		cDtPagExt	:= StrZero(Day(aCabec[1,7]),2) + ' de ' //" de "
		cDtPagExt	+= MesExtenso(Month(aCabec[1,7])) + ' de ' //" de "
		...
		nLin  += nSoma10
		oPrint:Line(nLin,030,nLin,575)	//Linha horizontal
		If Len(aCabec[1,1]) >= 14 .And. Len(aCabec[1,1]) >= 15
			nLin  += 15
		...
		EndIf
		If Len(aCabec[1,1]) >= 16 .And. Len(aCabec[1,1]) >= 17
			nLin  += nSoma10
