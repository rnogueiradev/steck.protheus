...
WSRESTFUL 4MDGAPI DESCRIPTION "API de Integra��o 4MDG"

WSMETHOD POST Fornecedores ;
    DESCRIPTION "M�todo de Cadastro de Fornecedores" ;
    WSSYNTAX "Fornecedores" ;
    PATH "Fornecedores" PRODUCES APPLICATION_JSON

WSMETHOD POST Produtos ;
    DESCRIPTION "M�todo de Cadastro de Produtos" ;
    WSSYNTAX "Produtos" ;
    PATH "Produtos" PRODUCES APPLICATION_JSON

WSMETHOD POST Clientes ;
    DESCRIPTION "M�todo de Cadastro de Clients" ;
    WSSYNTAX "Clientes" ;
...
If Len(Self:aURLParms) > 0
    ...
    If !Empty( cBody )
        ...
        If AllTrim( Upper( ValType( oJForn ) ) ) == "O" .Or. AllTrim( Upper( ValType( oJForn ) ) ) == "J"
            // Valida se Layout existe e est� preenchido...
            dbSelectArea("Z9A")
            dbSetOrder(1)
            Z9A->( dbGoTop() )
            ...
            If lLayout
                ...
                Begin Sequence 
                    ...
                    // Busca chave pelo �ndice Prim�rio do Layout... (CNPJ)
                    dbSelectArea("SA2")
                    ...
                    If aScan( aNomeFor, {|x| Alltrim( x ) == AllTrim( Z9B->Z9B_CHVIN1 ) } ) > 0
                        ...
                        If SA2->( dbSeek( xFilial("SA2") + oJForn:GetJsonText( AllTrim( Z9B->Z9B_CHVIN1 ) ) ) )
                            ...
                            cCodigo   := SA2->A2_COD
                            cTipo     := SA2->A2_TIPO
                            ConOut( "001 - C�digo..: " + SA2->A2_COD )
                            ConOut( "001 - Loja....: " + SA2->A2_LOJA )
                            ConOut( "001 - cCodigo.......: " + cCodigo )
                        Else
                            ConOut("CNPJ n�o encontrado " + oJForn:GetJsonText( AllTrim( Z9B->Z9B_CHVIN1 ) ))
                            nOperacao := 3
                            INCLUI    := .T.
                            ALTERA    := .F.
                            cCodigo   := ""
                    ...
                    // Busca chave pelo �ndice Alternativo do Layout... (CPF)
                    If aScan( aNomeFor, {|x| Alltrim( x ) == AllTrim( Z9B->Z9B_ALTIN1 ) } ) > 0 .And. Empty( cChave )
                        If !Empty( oJForn:GetJsonText( AllTrim( Z9B->Z9B_ALTIN1 ) ) )
                            ...
                            If dbSeek(xFilial("SA2") + oJForn:GetJsonText( AllTrim( Z9B->Z9B_ALTIN1 ) ) )
                                ...
                                cCodigo   := SA2->A2_COD
                                cTipo     := SA2->A2_TIPO
                                ConOut( "002 - C�digo..: " + SA2->A2_COD )
                                ConOut( "002 - Loja....: " + SA2->A2_LOJA )
                                ConOut( "002 - cCodigo.......: " + cCodigo )
                            Else
                                ConOut("CPF n�o encontrado " + oJForn:GetJsonText( AllTrim( Z9B->Z9B_ALTIN1 ) ))
                                nOperacao := 3
                                INCLUI    := .T.
                                ALTERA    := .F.
                                cCodigo   := ""
                    ...
                    Do While !Z9C->( Eof() ) .And. AllTrim( Z9C->Z9C_CADAST ) == "SA2"
                        ...
                        If AllTrim( Z9C->Z9C_CAMPO ) <> AllTrim( Z9B->Z9B_CPODEF ) .And. AllTrim( Z9C->Z9C_TRATAM ) <> "CHAVE" ;
                            ...
                            If AllTrim( Z9C->Z9C_CPO4MD ) <> "FIXO"
                                // Se membro n�o existir, pular o registro...
                                If aScan( aNomeFor, {|x| Alltrim( x ) == AllTrim( Z9C->Z9C_CPO4MD ) } ) > 0
                                    ...
                                    If SX3->( dbSeek( Z9C->Z9C_CAMPO ) )
                                        If SX3->X3_TIPO == "C"
                                            ...
                                            Else
                                                // Se caractere, truncar pois d� erro se conte�do for maior que X3_TAMANHO...
                                ...
                                Else
                                    ConOut("Membro da Tag n�o existe")
                                EndIf
                            Else
                                If !Empty( Z9C->Z9C_TRATAM )
                    ...
                    // Inclui campo C�digo do Munic�pio se encontrar...
...
//                                aAdd( aMensagem, {"Integracao Sucesso",IIf(Empty(cCodigo),cA2Cod,cCodigo),IIf(Empty(cCodigo),"Fornecedor incluido com sucesso","Fornecedor alterado com sucesso"),SA2->A2_LOJA})
                    ...
                    If ! lDeuCerto
                        ...
                        Else
                            ...
                            aAdd( aMensagem, {"Erro grava��o",cCodigo,cMensErro,""})
                            nStatusCode := 400
                            ConOut( "DEU ERRO!" )
                        EndIf
                    EndIf
            ...
            Else
                ...
                lRet 		:= .F.
                nStatusCode	:= 400
                cMessage    := "Dados inv�lidos!"
            EndIf
		EndIf
    Else
        aAdd( aMensagem, {"N�o h� dados","","Body vazia",""})
		lRet 		:= .F.
		nStatusCode	:= 400
		cMessage    := "N�o h� dados!"
	EndIf
EndIf
...
If Len( aMensagem ) == 0
    aAdd( aMensagem, {"Dados Inv�lido","","Falta membros obrigat�rios na body"})
EndIf
...
//	cBody := NoAcento( EncodeUTF8( SpecialChar( cBody ) ) )
    ...
    If !Empty( cBody )
        ...
        If AllTrim( Upper( ValType( oJProd ) ) ) == "O" .Or. AllTrim( Upper( ValType( oJProd ) ) ) == "J"
            ...
            // Alimentar subs que n�o foram previstas
            ...
            If nPosCod > 0
                ...
                If lLayout
                    ...
                    Begin Sequence 
                        ...
                        // Busca chave pelo �ndice Prim�rio do Layout...
                        If !Empty( cProximo )
                            dbSelectArea("SB1")
                            dbSetOrder( 1 )
                            ...
                            If SB1->( dbSeek( xFilial("SB1") + cChave ) )
                                ...
                                cCodigo   := SB1->B1_COD
                            Else
                                ConOut("Produto n�o encontrado " + cChave)
                                nOperacao := 3
                                INCLUI    := .T.
                                ALTERA    := .F.
                                cCodigo   := cChave
                        ...
                        /* --> Ser� adicionado no layout e no 4MDG dentro das subs das empresas...
                        If !Empty( cTipo )
                        ...
                        EndIf
                        */
                        If aScan( aCampos, {|x| AllTrim( x[1] ) == "B1_TIPO" } ) == 0 // Somente se tag n�o estiver no layout...
                            If !Empty( cTipo )
                                oSB1Mod:setValue("B1_TIPO"   , cTipo )
                            Else
                                oSB1Mod:setValue("B1_TIPO"   , "PA" )
                        ...
                        For nLin := 1 To Len( aCampos )
                            If !(AllTrim( aCampos[nLin][1] ) == "B1_COD" .And. AllTrim( aCampos[nLin][3] ) == "codigo") //.And. AllTrim( aCampos[nLin][1] ) <> "B1_TIPO"
                                // Se c�digo de barras Schneider traz cCodBar
                    ...
                    Begin Sequence 
                        ...
                        If aScan( aNomeFor, {|x| Alltrim( x ) == AllTrim( Z9B->Z9B_CHVIN1 ) } ) > 0
                            ...
                            If SA1->( dbSeek( xFilial("SA1") + oJCliente:GetJsonText( AllTrim( Z9B->Z9B_CHVIN1 ) ) ) )
                                ...
                                cCodigo   := SA1->A1_COD
                                cTipo     := SA1->A1_TIPO
                                ConOut( "001 - C�digo..: " + SA1->A1_COD )
                                ConOut( "001 - Loja....: " + SA1->A1_LOJA )
                                ConOut( "001 - cCodigo.......: " + cCodigo )
                            Else
                                ConOut("CNPJ n�o encontrado " + oJCliente:GetJsonText( AllTrim( Z9B->Z9B_CHVIN1 ) ))
                                nOperacao := 3
                                INCLUI    := .T.
                                ALTERA    := .F.
                                cCodigo   := ""
                        ...
                        If aScan( aNomeFor, {|x| Alltrim( x ) == AllTrim( Z9B->Z9B_ALTIN1 ) } ) > 0 .And. Empty( cChave )
                            If !Empty( oJCliente:GetJsonText( AllTrim( Z9B->Z9B_ALTIN1 ) ) )
                                ...
                                If dbSeek(xFilial("SA1") + oJCliente:GetJsonText( AllTrim( Z9B->Z9B_ALTIN1 ) ) )
                                    ...
                                    cCodigo   := SA1->A1_COD
                                    cTipo     := SA1->A1_TIPO
                                    ConOut( "002 - C�digo..: " + SA1->A1_COD )
                                    ConOut( "002 - Loja....: " + SA1->A1_LOJA )
                                    ConOut( "002 - cCodigo.......: " + cCodigo )
                                Else
                                    ConOut("CPF n�o encontrado " + oJCliente:GetJsonText( AllTrim( Z9B->Z9B_ALTIN1 ) ))
                                    nOperacao := 3
                                    INCLUI    := .T.
                                    ALTERA    := .F.
                                    cCodigo   := ""
                        ...
                        If oModel:VldData()
                            ...
                            If oModel:CommitData()
                                ...
                                ConOut( "DEU CERTO!" )
                                ConOut( "004 - C�digo..: " + SA1->A1_COD )
                                ConOut( "004 - Loja....: " + SA1->A1_LOJA )
                            //Se n�o deu certo, altera a vari�vel para false
                            Else
                                lDeuCerto := .F.
                                ConOut( "005 - C�digo..: " + SA1->A1_COD )
                                ConOut( "005 - Loja....: " + SA1->A1_LOJA )
                            EndIf
                        //Se n�o conseguir validar as informa��es, altera a vari�vel para false
                        Else
                            lDeuCerto := .F.
                            ConOut( "006 - C�digo..: " + SA1->A1_COD )
...
cString := StrTran(cString,"ASA£","ca")
cString := StrTran(cString,"A¡","a")
cString := StrTran(cString,"A£","a")
cString := StrTran(cString,"A¢","a")
cString := StrTran(cString,"A©","o")
...
cString := StrTran(cString,"A�A3","o")
cString := StrTran(cString,"A�Au","o")
cString := StrTran(cString,"A�AS","�")
cString := StrTran(cString,"ASAu","�o")
cString := StrTran(cString,"ASA£","��")
cString := StrTran(cString,"A¡","a")
cString := StrTran(cString,"A£","a")
cString := StrTran(cString,"A¢","a")
cString := StrTran(cString,"A©","o")
...
If lMoeda
    ...
    cString := StrTran(cString,"A�","a")
EndIf
cString := StrTran(cString,"ACA�� ","CA")
cString := StrTran(cString,"ACA��","CA")
cString := StrTran(cString,"ACA�a��","CO")
cString := StrTran(cString,"A�Š","E")
cString := StrTran(cString,"A�.","I")
cString := StrTran(cString,"A�.","A")
cString := StrTran(cString,"A�� ","A")
...
//cString := StrTran(cString,'""','"')
cString := StrTran(cString,"TRIFISICA","TRIF�SICA")
cString := StrTran(cString,"A�a��a��","-")
cString := StrTran(cString,"AA�a��šA2","2")
...
cString := StrTran(cString,"OPERA�A�a��ES","OPERACOES")
cString := StrTran(cString,"A�a��","O")
cString := StrTran(cString,",",".")

//cString := StrTran(cString,"CAO","�AO")
//cString := StrTran(cString,"COES","�OES")
//cString := StrTran(cString,"cao","�ao")
//cString := StrTran(cString,"coes","��es")

//cString := StrTran(cString,"AACAO","A�AO")
//cString := StrTran(cString,"AACOES","A�OES")
