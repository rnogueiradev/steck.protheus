...
/*/{Protheus.doc} STAnexoV
@description
Rotina generica para anexar documentos. Mesmo sendo uma inclus�o
@type function
@version  1.00
@author Valdemir Jose
@since 29/06/2021
@param pcAlias, variant, param_description - Alias da Tabela
@param pcINCLUI, variant, param_description - � uma Inclus�o
@param _SubPasta, variant, param_description  - SubPasta a ser salvo o documento
@param paCampos, variant, param_description - Campos a serem utilizados na interface
@return variant, return_description - Sempre .T.
/*/
User Function STAnexoV(pcAlias, _SubPasta, paCampos, plObrig, pDOC)
	Local aArea       := GetArea()
	Local aArea1      := (pcAlias)->(GetArea())
	Local _aCampos    := aClone(paCampos)
	Local n           := 0
	Local lSaida      := .f.
	Local nOpcao      := 0
	Local oDxlg
    ...
    bOK := {|| lValid := (!Empty(_cAne01) .or. !Empty(_cAne02) .or.!Empty(_cAne03) .or.!Empty(_cAne04) .or.!Empty(_cAne05)) }

	if Len(paCampos)==0
		FWMsgRun(,{|| Sleep(3000)},"Informativo","Campos obrigat�rios para montagem da tela n�o informado")
		Return
	Endif

	dbSelectArea(pcAlias)

	cSolicit  := (pcAlias)->&(_aCampos[2])
	_cNUm     := cSolicit
	...
	If ExistDir(_cServerDir)
		...
		if Right(_aCampos[2],4)=="USER"
			cNameSolic := USRRETNAME((pcAlias)->&(_aCampos[2]))
		else
			cNameSolic := (pcAlias)->&(_aCampos[3])
		endif

		Do While !lSaida
			nOpcao := 0

			Define msDialog oDxlg Title "Selecione os Anexos [Documento: "+pDOC+" ]" From 10,10 TO 450,600 Pixel
			_nLin := 005
			@ _nLin,010 say "Filial" COLOR CLR_BLACK  Of oDxlg Pixel
			@ _nLin,070 get xFilial(pcAlias) when .f. size 050,08  Of oDxlg Pixel
			...
			DEFINE SBUTTON FROM _nLin,130 TYPE 1 ACTION (eVal(bOK), ConfOK(oDxlg, plObrig,@lSaida) ) ENABLE OF oDxlg
...
Static Function ConfOK(pDLG, plObrig, plSaida)
	...
	if plObrig
		if lValid
			...
			MsgInfo('Obrigat�rio Adicionar um anexo','Aten��o!')
		endif
	else
        plSaida := .T.
		pDLG:End()
...
@author Valdemir Jose
@since 29/06/2021
@param _nSave, variant, param_description
@param _cFile, variant, param_description
@param cSolicit, character, param_description
...
/*/{Protheus.doc} DelAnexo
description
@type function
@version  
@author Valdemir Jose
@since 29/06/2021
@param _nDel, variant, param_description
@param _cFile, variant, param_description
@param cSolicit, character, param_description
@return variant, return_description
/*/
...
@author Valdemir Jose
@since 29/06/2021
@param _nOpen, variant, param_description
@param _cFile, variant, param_description
@param cSolicit, character, param_description
@param _SubPasta, variant, param_description
@return variant, return_description
/*/
