...
±±³Programa  ³ BOLCITI  ³ Autor ³ Everson Santana       ³ Data ³ 16/12/19 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ IMPRESSAO DO BOLETO BANCO CITIBANK  COM CODIGO DE BARRAS   ³±±
±±³			Faturamento												  	   ±±³
...
User Function STCITI04() //U_STCITI04
	LOCAL	aPergs := {}
	PRIVATE lExec    := .F.
	PRIVATE cIndexName := ''
	PRIVATE cIndexKey  := ''
	...
	If lExec
		...
		Do While !EOF()
			If Marked("E1_OK")
				...
				If SE1->(Eof())
					SE1->(DbSkip(-1))
					U_MTCITIR(,,,,.T.,MV_PAR01,MV_PAR02,MV_PAR03,MV_PAR01,,,.t.,_cCaminho,_aParcs)
					_aParcs := {}
				Else
				...
				//U_MTCITIR(,,,,.T.,MV_PAR01,MV_PAR02,MV_PAR03,MV_PAR01,,,.t.,_cCaminho)
				MsUnLockAll()
				//>> VOLTAR
				/*
				...
				StartJob("U_EnvBolCiti",GetEnvServer(), .F.,"\arquivos\BOLETOS_CITI\" , MV_PAR01+ "_BOLETO.pdf",MV_PAR19)

				Else
				MsgInfo("E-mail Vazio........!!!!!!!!!!!")
				EndIf
				*/
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³  MTCITIR³ Autor ³ Microsiga             ³ Data ³ 13/10/03 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function MTCITIR(xBanco,xAgencia,xConta,xSubCt,lSetup,cNotIni,cNotFim,cSerie,cChave,_cCliente,_cLoja,_lImpPr,_cCaminho,_aParcs)
...
aDadosEmp    := {	SM0->M0_NOMECOM                                    ,; //[1]Nome da Empresa
	...
	_cQuery += " AND E1_SALDO>0 AND E1_PORTADO='745' "
	...
	cNroDoc :=  " "
	cStartPath 	  	:= "\arquivos\BOLETOS_CITI\"
	_cNomePdf     	:= AllTrim(cChave)+ '_BOLETO'
	If _lImpPr
		//_cDirRel      	:=  'C:\ARQUIVOS_PROTHEUS\BOLETOS_CITI\'+cEmpAnt
	...
	Do While (_cAlias)->(!EOF())
		...
		_nMoll:=  Round(( 6 * _nMoll/30/100),2)
		...
		If Empty(Alltrim(SA6->A6_DVCTA))
			...
			SUBSTR(SA6->A6_AGENCIA, 1, 3)                          ,; 	// [3]Agência
		...
		aCB_RN_NN    := Ret_cBarra(Subs(aDadosBanco[1],1,3),aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],cNroDoc,(E1_VALOR-nVlrAbat),E1_VENCTO,aDadosBanco[7])
...
Static Function Impress(oPrint,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,aCB_RN_NN,_cParcela)
	...
	*/
	...
	oPrint:Say  (nRow1+0075+(oFont21:nHeight*2),513,aDadosBanco[1]+"-5",oFont21 )		// [1]Numero do Banco
	...
	oPrint:Say  (nRow1+0150+(oFont8:nHeight*2),1060,"Agência/Cód Cedente/Beneficiário",oFont8)
	//oPrint:Say  (nRow1+0200+(oFont10:nHeight*2),1060,aDadosBanco[3]+"/"+aDadosBanco[4]+"-"+aDadosBanco[5],oFont10)
	oPrint:Say  (nRow1+0200+(oFont10:nHeight*2),1060,aDadosBanco[3]+"/"+aDadosBanco[7],oFont10)

	oPrint:Say  (nRow1+0150+(oFont8:nHeight*2),1510,"Nro.Documento",oFont8)
	oPrint:Say  (nRow1+0200+(oFont9n:nHeight*2),1510,AllTrim(_cParcela)+" "+aDadosTit[1],oFont9n)
	...
	oPrint:Say  (nRow2+0635+(oFont21:nHeight*2),513,aDadosBanco[1]+"-5",oFont21 )	// [1]Numero do Banco
	...
	oPrint:Say  (nRow2+0810+(oFont8:nHeight*2),1810,"Agência/Cód Cedente/Beneficiário",oFont8)
	//cString := Alltrim(aDadosBanco[3]+"/"+aDadosBanco[4]+"-"+aDadosBanco[5])
	cString := Alltrim(aDadosBanco[3]+"/"+aDadosBanco[7])
	...
	oPrint:Say  (nRow2+0940+(oFont10:nHeight*2),1050,"00"										,oFont10) //Tipo do Titulo

	oPrint:Say  (nRow2+0910+(oFont8:nHeight*2),1305,"Aceite"                                         ,oFont8)
	oPrint:Say  (nRow2+0940+(oFont10:nHeight*2),1400,"N"                                             ,oFont10)
	...
	oPrint:Say  (nRow2+1536+(oFont10:nHeight*2),1850,Alltrim(Substr(aDadosTit[6],1,11)) ,oFont10)
	...
	oPrint:Say  (nRow3+1925+(oFont21:nHeight*2),513,aDadosBanco[1]+"-5",oFont21 )	// 	[1]Numero do Banco
	...
	oPrint:Say  (nRow3+2100+(oFont8:nHeight*2),1810,"Agência/Cód Cedente/Beneficiário",oFont8)
	cString := Alltrim(aDadosBanco[3]+"/"+aDadosBanco[7])
	...
	oPrint:Say  (nRow3+2230+(oFont10:nHeight*2),1050,"00"										,oFont10) //Tipo do Titulo

	oPrint:Say  (nRow3+2200+(oFont8:nHeight*2),1305,"Aceite"                                         ,oFont8)
	oPrint:Say  (nRow3+2230+(oFont10:nHeight*2),1400,"N"                                             ,oFont10)
	...
	oPrint:Say  (nRow3+2780+(oFont10:nHeight*2),1750,Alltrim(Substr(aDadosTit[6],1,11))  ,oFont10)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function Ret_cBarra(cBanco,cAgencia,cConta,cDacCC,cNroDoc,nValor,dVencto,cCCosmo)
	...
	//	  X = DAC que amarra o campo, calculado pelo Modulo 10 da String do campo (Dígito verificador do 1º campo da linha digitável (ver pág. 20))
...
User Function EnvBolCiti(cStartPath,_cNomePdf,cEmail)
	local _aAttach  := {}
