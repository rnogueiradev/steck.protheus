...
/*/{Protheus.doc} STAV3603

Consulta Padrão para retornar o Dados do Avaliador(AV3603)
...
User Function STAV3603()
	...
	Private _nColuna 	:= 2
	cQuery := " SELECT * FROM ( "
	cQuery += " SELECT RA_XEMP EMP, RA_FILIAL FILIAL, RA_MAT MATRICULA,RA_NOME NOME,RA_XUSRCFG USRAVA FROM SRA010 WHERE RA_DEMISSA = ' ' AND D_E_L_E_T_ = ' ' "
	cQuery += " UNION "
	cQuery += " SELECT RA_XEMP EMP, RA_FILIAL FILIAL, RA_MAT MATRICULA,RA_NOME NOME,RA_XUSRCFG USRAVA FROM SRA030 WHERE RA_DEMISSA = ' '  AND  SUBSTR(RA_MAT,1,1) <> '9' AND D_E_L_E_T_ = ' ' "
	cQuery += " UNION "
	cQuery += " SELECT RA_XEMP EMP, RA_FILIAL FILIAL, RA_MAT MATRICULA,RA_NOME NOME,RA_XUSRCFG USRAVA FROM SRA070 WHERE RA_DEMISSA = ' ' AND D_E_L_E_T_ = ' ' "
	...
	If Len(aRet) > 0 .And. lRet
		...
		Else
			...
			SetMemVar("Z33_MATAVA",cRETGERAV)
			SetMemVar("Z33_XEMP",cRETGERAV1)
			SetMemVar("Z33_XFILIA",cRETGERAV2)
			SetMemVar("Z33_NOME",cRETGERAV3)
			SetMemVar("Z33_USRAVA",cRETGERAV4)
...
Return lRet
	...
	*********************************************************************
User Function RETGERAV()
Return (cRETGERAV) 

User Function RETGERA1()
Return (cRETGERAV1) 

User Function RETGERA2()
Return (cRETGERAV2)

User Function RETGERA3()
Return (cRETGERAV3) 

User Function RETGERA4()
Return (cRETGERAV4) 
