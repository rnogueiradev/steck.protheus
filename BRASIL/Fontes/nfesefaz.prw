...
//Declaração de Arrays
	...
	Local aObsItem		:= {}
	Local aNFOri    	:={}				// steck
	Static _nRet		:= 0				// steck

//Declaração de Strings
	Local cxMenCli   	:= ""       // STECK
	Local cString    	:= ""
	Local cNatOper   	:= ""
	Local cModFrete  	:= ""
	Local cScan      	:= ""
...
//INICIO CUSTOMIZAÇÃO STECK
	local aAuxDi  := {}
//FIM CUSTOMIZAÇÃO STECK
...
//Declaração de Lógicos
	...
	Private lServDf		:= .F.
	Private _cResSte   	:= ' ' //STECK
	Private _lDci		:= .F. //STECK
	Private cMensTNT 	:= ' ' //STECK
			...
			If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				If MsSeek(xFilial("SF3")+cClieFor+cLoja+cNota+cSerie)
					While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
						...
						// steck abre MSG COMPLEMENTAR ICMS 4%
						If SD2->D2_PICM==4.00
							_nRet	:=	1
						Endif
						// steck fecha MSG COMPLEMENTAR ICMS 4%
						...
						cFormulaX := IIf(Empty(SF4->F4_FORMULA), IIf(Empty(cFormulaX), "", cFormulaX), SF4->F4_FORMULA)
				...
				Else
					...
					//STECK GIOVANI ZAGO 23/03/15 TRATAMENTO PARA MANDAR O ENDEREÇO DE ENTREGA TNT
					If SD2->(DbSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA))
						...
						If SC5->(DbSeek(xFilial("SC5")+SD2->D2_PEDIDO))
							If !(Empty(Alltrim(SC5->C5_ZENDENT))) .Or. 	!(Empty(Alltrim(SC5->C5_ZCEPE)))
								//If SF2->F2_TRANSP = '000163'
									If GetMv("ST_NFECEP",,.F.)
										aadd(aEntrega,SA1->A1_CGC)//01
										aadd(aEntrega,MyGetEnd(SC5->C5_ZENDENT,"SA1")[1])//02
										aadd(aEntrega,ConvType(IIF(MyGetEnd(SC5->C5_ZENDENT,"SA1")[2]<>0,MyGetEnd(SC5->C5_ZENDENT,"SA1")[2],"SN")))//03
										aadd(aEntrega,MyGetEnd(SC5->C5_ZENDENT,"SA1")[4])//04
										aadd(aEntrega,SC5->C5_ZBAIRRE)//05
										aadd(aEntrega,SC5->C5_XCODMUN)//06
										aadd(aEntrega,SC5->C5_ZMUNE)//07
										aadd(aEntrega,Upper(SC5->C5_ZESTE))//08
										aadd(aEntrega,SA1->A1_NOME)//09
										aadd(aEntrega,SA1->A1_INSCR)//10
										//>>
										aadd(aEntrega,Alltrim(SA1->A1_CEP))//11
										aadd(aEntrega,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP")))//12
										aadd(aEntrega,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" )))//13
										aadd(aEntrega,Alltrim(SA1->A1_DDD+SA1->A1_TEL)) //14
										aadd(aEntrega,Alltrim(SA1->A1_EMAIL))//15
										//<<
									EndIf
								//EndIf
								//		EndIF
					...
					*/
					...
					If MsSeek(cChave)
						...
						If AllTrim(SF3->F3_CFO) == "5111" .Or. AllTrim(SF3->F3_CFO) == "6111" .or.  AllTrim(SF3->F3_CFO)  == '5918' .or.  AllTrim(SF3->F3_CFO)  == '6918' .Or. AllTrim(SF3->F3_CFO)  == '5114' .Or.  AllTrim(SF3->F3_CFO)  == '5113'
							lConsig  := .T.
					...
					MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
					While !Eof() .And. xFilial("SF3") == SF3->F3_FILIAL .And.;
							SF2->F2_SERIE == SF3->F3_SERIE .And.SF2->F2_DOC == SF3->F3_NFISCAL .AND. !EOF()
						If !SF3->F3_DESCZFR == 0
							_nValObse += SF3->F3_VALOBSE
						Endif
						dbskip()
					Enddo
					//steck fecha 2
					...
					#IFDEF TOP
						...
						IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
							BeginSql Alias cAliasSD2
								...
								D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_BRICMSO,D2_ICMRETO,D2_BRICMSD,D2_ICMRETD,D2_EST,D2_CSOSN,D2_VALICM %Exp:cField% 
								FROM %Table:SD2% SD2
								WHERE
						...
						Else
							...
							D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
							D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
							D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_CSOSN,D2_EST,D2_VALICM %Exp:cField% 
							FROM %Table:SD2% SD2
							WHERE
							SD2.D2_FILIAL  = %xFilial:SD2% AND
							SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
					...
					If SubStr((cAliasSD2)->D2_CF,2,3)=="114" //Especifico Selia Ticket 20220523010700
                          lIcmDevol := .f.
						  lIcmSTDev := .f.
					Endif
					...
					If lContinua
			    		...
			    		Endif
							...
							//steck abre 3
							If !Empty(SF4->F4_MENSP1) .And. !AllTrim(FORMULA(SF4->F4_MENSP1)) $ cMensCli
								cMensCli += AllTrim(FORMULA(SF4->F4_MENSP1))+", "
							EndIf
							If !Empty(SF4->F4_MENSP2) .And. !AllTrim(FORMULA(SF4->F4_MENSP2)) $ cMensCli
								cMensCli += AllTrim(FORMULA(SF4->F4_MENSP2))+", "
							EndIf
							If !Empty(SF4->F4_MENSP3) .And. !AllTrim(FORMULA(SF4->F4_MENSP3)) $ cMensCli
								cMensCli += AllTrim(FORMULA(SF4->F4_MENSP3))+", "
							EndIf
							//steck fecha 3
						...
						EndIF
							...
							//STECK GIOVANI ZAGO
							If cEmpAnt = '03' .And.  !("DCI MENSAL" $ cInfAdic)
								If SB1->B1_DCI = '1'
									_lDci:= .T.
								EndIf
							EndIf
						...
						*/
								...
								//steck abre
								If !Empty(SC5->C5_XTPFRET)
									cModFrete := SC5->C5_XTPFRET
								EndIf
								//steck fecha

								If Empty(aPedido)
									//Steck abre nao enviar pedcli giovani zago 01/12/16
								...
								//> Ticket 20200129000205 steck abre
								If lEECFAT .AND. (cAliasSD2)->D2_EST $ "EX" //Incluir na query o campo D2_EST

									cDescProd := Alltrim(Posicione("SB5",1,xFilial("SB5")+cCodProd,"B5_XDESCEX"))

									If Empty(cDescProd)
										cDescProd := AllTrim(SB1->B1_XDESEXD)
										If Empty(cDescProd)
								...
								EndIf
								//<< 20200129000205 20200129000205 steck fecha
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "SOL"
									...
									If cVerAmb == "4.00" .and. nValST > 0 .And. lConsig .And. !(Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM)  //Especifico Selia Ticket 20220523010700
										nSTConsig += nValST
									EndIf
						...
						*/
							...
							//steck abre 4
							If _nValObse > 0
								//cMenscli :="ISENTO DO ICMS CONF ART 84, ANEXO I, DECR 45490/00, VALOR ICMS 7% "+alltrim(str(_nValObse-SF2->F2_DESCONT-nValPisZF-nValCofZF,13,2))+",ALIQUOTAS REDUZIDAS A ZERO PARA COFINS E PIS CONF. ARTIGO 2 DA LEI 10996/2004 SUFRAMA:"+SA1->A1_SUFRAMA
								//cMenscli +="VALOR ABAT.: " +alltrim(str(nValPisZF+nValCofZF,13,2))+" PIS/COFINS: 9.25%"
								cMenscli :="ISENTO DO ICMS CONF ART 84, ANEXO I, DECR 45490/00, VALOR ICMS 7% "+CVALTOCHAR(Round(((SF2->F2_VALMERC+SF2->F2_DESCZFR)-(SF2->F2_VALMERC+SF2->F2_DESCZFR)*((getmv("MV_TXPIS")+getmv("MV_TXCOFIN"))/100))*(7/100),2)-STRETIMP())+", ALIQUOTAS REDUZIDAS A ZERO PARA COFINS E PIS CONF. ARTIGO 2 DA LEI 10996/2004 SUFRAMA:"+SA1->A1_SUFRAMA
								cMenscli +="VALOR ABAT.: " +CVALTOCHAR(Round((SF2->F2_VALMERC+SF2->F2_DESCZFR)*((getmv("MV_TXPIS")+getmv("MV_TXCOFIN"))/100),2))+" PIS/COFINS: 9.25%"
							Endif
							//steck fecha 4
							//steck abre    Giovani Zago Endereço de Entrega 23/01/14
							dbSelectArea("SD2")
							SD2->(dbSetOrder(3))
							If SD2->(DbSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)) .And. !(cEmpAnt = '03')
								dbSelectArea("SC5")
								SC5->(dbSetOrder(1))
								If SC5->(DbSeek(xFilial("SC5")+SD2->D2_PEDIDO))

									If !Empty(SC5->C5_XORDEM) .And. !("OC: "+AllTrim(SC5->C5_XORDEM)+" " $ cMensCli)
										cMenscli += "OC: "+AllTrim(SC5->C5_XORDEM)+" "
									EndIf

                            IF SC5->(FieldPos("C5_ZMENAGL")) > 0
						       If !AllTrim(SC5->C5_ZMENAGL) $ cMensCli
                                  cMensCli+= ' '+ALLTRIM(SC5->C5_ZMENAGL)+' '
							   ENDIF
						    ENDIF	 

							cMenscli += "OS: "+GETORDSEP()+" "
									If !(Empty(Alltrim(SC5->C5_ZENDENT))) .Or. 	!(Empty(Alltrim(SC5->C5_ZCEPE)))
										If SF2->F2_TRANSP <> '000163'
											cMenscli +=" END DE ENTREGA: " + Alltrim(SC5->C5_ZENDENT)+" - "+ Alltrim(SC5->C5_ZMUNE)+" - "+ Alltrim(SC5->C5_ZBAIRRE)+;
												" - " + Alltrim(SC5->C5_ZESTE) + " - " + Alltrim(SC5->C5_ZCEPE) + " - " + Alltrim(SC5->C5_XCODMUN)
										Else
											If GetMv("ST_NFECEP",,.F.)
												cQuery := 'SELECT DISTINCT CB6_XORDSE "OS",(SELECT COUNT(*) FROM CB6010 CB6 WHERE   CB7_ORDSEP = CB6_XORDSE
												cQuery += " AND CB6.CB6_FILIAL = '"+xFilial("CB6")+"' AND  CB6.D_E_L_E_T_ = ' ' )
												cQuery += ' "SOMA"
												cQuery += " FROM CB7010 CB7
												cQuery += " INNER JOIN(SELECT * FROM CB6010)CB6
												cQuery += " ON   CB7_ORDSEP = CB6_XORDSE AND  CB6.D_E_L_E_T_ = ' ' AND CB6.CB6_FILIAL = '"+xFilial("CB6")+"'"
												cQuery += " WHERE CB7.D_E_L_E_T_ = ' '  AND CB7.CB7_FILIAL = '"+xFilial("CB7")+"'"
												cQuery += " AND CB7.CB7_NOTA     = '"+SF2->F2_DOC+"'"
												cQuery += " AND CB7.CB7_SERIE    = '"+SF2->F2_SERIE+"'"
												cQuery := ChangeQuery(cQuery)
												_cResSte := __CUSERID + SUBSTR(cTime, 1, 2)+ SUBSTR(cTime, 4, 2)+ SUBSTR(cTime, 7, 2)
												If Select(_cResSte) > 0
													(_cResSte)->(dbCloseArea())
												EndIf
												dbUseArea(.T.,'TOPCONN',TcGenQry(,,cQuery),_cResSte,.T.,.T.)
												cxMenCli:= cMenscli
												cMenscli := PADR( "CEP:"+ PADL(Alltrim(SC5->C5_ZCEPE) ,08,'0'),87,' ')
												DbSelectArea(_cResSte)
												(_cResSte)->(dbgotop())
												cMensTNT := " --TNTOSVOL "
												While !(_cResSte)->(Eof())
													_nVol++
													If _nVol > 1
														cMensTNT += ","
													ENdIf
													cMensTNT += (_cResSte)->OS+"["+cValToChar((_cResSte)->SOMA)+"]"
													(_cResSte)->(dbSkip())
												End
												cMensTNT += " --ENDTNTOSVOL "
												cMensTNT += "   "+ Alltrim(cxMenCli)
											EndIf

											cMenscli += PADR( cMensTNT ,87,' ')
											cMenscli +="  END DE ENTREGA: " + Alltrim(SC5->C5_ZENDENT)+" - "+ Alltrim(SC5->C5_ZMUNE)+" - "+ Alltrim(SC5->C5_ZBAIRRE)+;
												" - " + Alltrim(SC5->C5_ZESTE) + " - " + Alltrim(SC5->C5_ZCEPE) + " - " + Alltrim(SC5->C5_XCODMUN)
										EndIf
									Else
										...
										Else
											If GetMv("ST_NFECEP",,.F.)
												...
												dbUseArea(.T.,'TOPCONN',TcGenQry(,,cQuery),_cResSte,.T.,.T.)
												cxMenCli:= cMenscli
												//cMenscli := "CEP:"+ PADL(Alltrim(SC5->C5_ZCEPE) ,08,'0')
												DbSelectArea(_cResSte)
												(_cResSte)->(dbgotop())
												cMensTNT += " --TNTOSVOL "
												While !(_cResSte)->(Eof())
												...
												cMensTNT += " --ENDTNTOSVOL "
												cMenscli += PADR( cMensTNT ,87,' ')
												cMenscli += "   "+ Alltrim(cxMenCli)
											EndIf
										EndIf
									EndIf
									// GIOVANI ZAGO CHAMADO 004277 VERONICA
									If GetMv("ST_BLQNF",,.T.)
										dbSelectArea("SA1")
										SA1->(dbSetOrder(1))
										If 	SA1->(DbSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
											If SA1->A1_CONTRIB = '2' .And. !(Empty(Alltrim(SC5->C5_ZCEPE))) .And. (Alltrim(SA1->A1_GRPVEN) $ ('E1/E3')) .And. Alltrim(SA1->A1_CEP) <>  Alltrim(SC5->C5_ZCEPE)
												cMenscli += "        Entrega por conta e ordem do destinatário: Ajuste SINIEF nº 1, de 21.03.2014"
											EndIf
										EndIf
									EndIf
									// GIOVANI ZAGO CHAMADO 004277 VERONICA
									// GIOVANI ZAGO CHAMADO 004806 VERONICA
									If GetMv("ST_BLQNF",,.T.)
										dbSelectArea("SA1")
										SA1->(dbSetOrder(1))
										If 	SA1->(DbSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
											If !(Empty(Alltrim(SA1->A1_FORMVIS)))
												DbSelectArea("SM4")
												DbSetOrder(1)
												DbSeek(xFilial("SM4")+SA1->A1_FORMVIS)
												If !SM4->(Eof())
													cMenscli += "         "+ AllTrim(SM4->M4_DESCR)+": "+AllTrim(StrTran(SM4->M4_FORMULA,'"',""))
												EndIf
											EndIf
										EndIf
									EndIf
								...
								Else
									If !Empty(cFormulaX)
										DbSelectArea("SM4")
										DbSetOrder(1)
										If DbSeek(FWxFilial("SM4") + cFormulaX)
											cMenscli += "         " + AllTrim(SM4->M4_DESCR) + ": ";
												+ AllTrim(StrTran(SM4->M4_FORMULA,'"',""))
										EndIf
									EndIf
								EndIf
							EndIf
							...
							If nValTFecp > 0
								...
								EndIf
							EndIf
							// Ricardo Minoro chamado 20230511005962
							if SF2->F2_XCUBAGE > 0
								cMenscli += "   "+ 'Cubagem : '+TRANSFORM( SF2->F2_XCUBAGE, "@E 999,999.99" )
							ENDIF
						EndIf
					EndIf
				Next
									...
									If (cOrigem $"1-2-3-4-5-6-8" .And. cCSTrib $ "00-10-20-30-40-41-50-51-60-70-90")
										If (cAliasSD1)->(ColumnPos("D1_FCICOD")) > 0 .And. !Empty((cAliasSD1)->D1_FCICOD)
											...
											If lFCI
												...
												cInfAdic  += cMsgFci + ", Numero da FCI " + Upper(Alltrim((cAliasSD2)->D2_FCICOD)) //steck
											EndIf
										Else
											aadd(aFCI,{})
										EndIf
					...
					*/
									...
									If lEasy  .And. ( !Empty((cAliasSD1)->D1_TIPO_NF) .Or. ( Empty((cAliasSD1)->D1_TIPO_NF) .And. (cAliasSD1)->D1_TIPO $ 'IPC' )   )
										...
										If !cTipoNF$"IPC" .And. cTipoNFEnt <> "6"
											...
											//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
											aAuxDi := (GetNFEIMP(.F.,cDocEnt,cSerEnt,cFornece,cLojaEnt,cTipoNFEnt,cPedido,cItemPC,cLote,cItem))
											aadd(aDI, aClone(aAuxDi))
										Else
											//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
											...
											EndIf
											if cTipoNFEnt == '6'
												aAuxDi :=(GetNFEIMP(.F.,cDocEnt,cSerEnt,cFornece,cLojaEnt,cTipoNFEnt,cPedido,cItem,cLote, ))
												aadd(aDI,aClone(aAuxDi))
											else
												aAuxDi := (GetNFEIMP(.F.,cNFOri,cSerOri,cFornece,cLojaEnt,cTipoNFEnt, ,cItemOri, ,cItem ))
												aadd(aDI,aClone(aAuxDi))
											EndIf
										EndIf
										aAdi := aDI
									...
									If !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO $ "DCPI") .or. (lEasy .and. empty(aAuxDi))
										DbSelectArea("CD5")
										DbSetOrder(4)
...
//Tratamento para que se caso mude o cliente o mesmo busque dados do cliente da nota original.
								...
								//steck abre 6
								If len(anfori) >0
									For nX := 1 to Len( aNFOri )
										cmenscli += "NF Origem: "+ anfori[ nX , 1 ] +" Serie Origem: "+ anfori[ nX , 2 ]
									Next nX
								endif
								//steck fecha 6
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	*/
	...
	//Steck abre
	_cEmail	:= ' '//'xml24h@xml24h.com.br'
	Do Case
	Case SA1->A1_COD=="008724" //PJ
		_cEmail	+= ';cx.nfe.eletricapj@unicasolucoes.com.br'
	Case SA1->A1_COD=="010566" //Neblina
		_cEmail	+= ';cx.nfe.neblina@unicasolucoes.com.br'
	EndCase


	if !lBrinde
		//cString += NfeTag('<EMAIL>',AllTrim(aDest[16])+cMailTrans)
		cString += NfeTag('<EMAIL>',AllTrim(aDest[16])+cMailTrans+IIf(SubStr(AllTrim(aDest[16])+cMailTrans,Len(AllTrim(aDest[16])+cMailTrans),1)==";",_cEmail,';'+_cEmail)) //Steck - E-mail ITGS
	endif
...
//Local lSimpNac   	:= SuperGetMV("MV_CODREG")== "1" .Or. SuperGetMV("MV_CODREG")== "2" 
	...
	Local lNfCpm		:= .F.
	Local _cXord 		:= ' ' //steck chamado janaina 24/08/16
	Local xQtd := 0
//Será retirado com o tempo .. Servirá apenas para contornar a Rejeição 630
	Local _cEmail := "fabio.barros@steck.com.br;felipe.falconi@steck.com.br"
	Local _cAssunto := ""
	Local cMsg := ""
...
//Validação para controle das tags que deverão ser preenchidas apenas nas operações não destinadas a consumidor final RS
	...
	If aProd[12] > 0
		If !(Val(ConvType(aProd[10],15,2)) == Round(Val(ConvType(aProd[12], 15, 4)) * Val(ConvType(aProd[10]/aProd[12],21,8)),2))
			xQtd := aProd[12]
			If Val(ConvType(aProd[10],15,2)) > Round(Val(ConvType(aProd[12], 15, 4)) * Val(ConvType(aProd[10]/aProd[12],21,8)),2) .And. (Val(ConvType(aProd[10],15,2)) - Round(Val(ConvType(aProd[12], 15, 4)) * Val(ConvType(aProd[10]/aProd[12],21,8)),2) > 0.01 )
				Do While !(Val(ConvType(aProd[10],15,2)) == Round(xQtd * Val(ConvType(aProd[10]/aProd[12],21,8)),2))
//			xQtd := xQtd + 0.00001
					xQtd := Round(Val(ConvType(aProd[10],15,2)) / Val(ConvType(aProd[10]/aProd[12],21,8)),8)
				EndDo
				aProd[12] := xQtd

				_cAssunto := "[WFPROTHEUS] - Verificar Fator Conversão para a 2a Unidade de Medida (KG), Produto: " + ConvType(aProd[02])
				cMsg += '<html><head><title>' + ConvType(aProd[02]) + " / " + ConvType(cEan) + '</title></head><body>'
				cMsg += 'Produto: ' + ConvType(aProd[02]) + ", EAN: " + ConvType(cEan) + '</b><br><br>'
				cMsg += 'Favor analisar para não termos problemas com Rejeição 630/SEFAZ<br><br>'
				cMsg += 'Atenciosamente,<br>'
				cMsg += 'Steck Indústria Elétrica Ltda'
				cMsg += '</body></html>'
				U_STMAILTES(_cEmail,"",_cAssunto,cMsg,"")

			ElseIf Val(ConvType(aProd[10],15,2)) < Round(Val(ConvType(aProd[12], 15, 4)) * Val(ConvType(aProd[10]/aProd[12],21,8)),2) .And. (Round(Val(ConvType(aProd[12], 15, 4)) * Val(ConvType(aProd[10]/aProd[12],21,8)),2) - Val(ConvType(aProd[10],15,2)) > 0.01 )
				Do While !(Val(ConvType(aProd[10],15,2)) == Round(xQtd * Val(ConvType(aProd[10]/aProd[12],21,8)),2))
					xQtd := xQtd - 0.00001
//			xQtd := Round( Val(ConvType(aProd[10]/aProd[12],21,8)) / Val(ConvType(aProd[10],15,2)),8)
				EndDo
				aProd[12] := xQtd
...
//Ticket 20221101020016 - Cinthya
	If cEmpAnt <> "11" .And. aDest[9] == "EX" .And. "." $ cvaltochar(aProd[12]) .And. (Len(cValToChar(aProd[12])) - At(".",cValToChar(aProd[12]),1)) > 3
		cAuxQTrib := cValToChar(aProd[12])
		If Substr(cAuxQTrib,Len(cAuxQTrib),1) > "5"
			cAuxQTrib := Substr(cAuxQTrib,1,Len(cAuxQTrib) - 1) + "0"
			aProd[12] := Val(cAuxQTrib) + 0.0010
		Else
			cAuxQTrib := Substr(cAuxQTrib,1,Len(cAuxQTrib) - 1) + "0"
			aProd[12] := Val(cAuxQTrib)
		EndIf
	EndIf
...
If !Empty(aFCI)
	...
	cString += '<nFCI>'+Upper(Alltrim(aFCI[01]))+'</nFCI>' //Steck
EndIf
cString += '</prod>'
DbSelectArea("SF4")
						...
						If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
							...
							If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP"
								...
								if Empty(At("Artigo 274 do RICMS", cMensFis))
									...
									//Padrão Totvs - envio da parcela cobrável, manter essa linha.
									...
									//TAG Xml 27/05/2022 não destacar antecipação  Ticket 20220527011050
									...
									cMensFis := AllTrim(cMensFis) + "|"+ConvType(aProd[02])+"|"+AllTrim(str(IcmsCbr(aNota,aProd),15,2))+"|&"
									//cMensFis += "Imposto Recolhido por Substituição - Artigo 274 do RICMS (Lei 6.374/89, art.67,Paragrafo 1o., e Ajuste SINIEF-4/93',cláusa terceira, na redação do Ajuste SINIEF-1/94) &|"+ConvType(aProd[02])+"|"+AllTrim(str(IcmsCbr(aNota,aProd),15,2))+"|&"
								endif
...
// Valor dos Tributos por Ente Tributante: Federal, Estadual e Municipal
			If lMvEnteTrb
				...
				If cMvMsgTrib $ "2-3" .And. cTpCliente == "F" .And. ( ( aProd[35] + aProd[36] + aProd[37] ) > 0 )
					...
					lProdItem	:= .T.
					//steck
					cCrgTrib	:= ' / '+' Lei da Transparência conforme Lei Federal nº 12.741/12 '+'Valor aproximado do(s) Tributo(s): '
			...
			Else
				...
				If aProd[30] > 0 .And. cMvMsgTrib $ "2-3" .And. cTpCliente == "F"
					...
					cCrgTrib := ' / '+' Lei da Transparência conforme Lei Federal nº 12.741/12 '+'Valor Aproximado dos Tributos: R$ '+ ConvType(aProd[30],15,2)+ " ("+cPercTrib+"%)."
				EndIf
...
// Valor dos tributos por Ente Tributante
			If lMvEnteTrb
				...
				If cMvMsgTrib $ "1-3" .And. cTpCliente == "F" .And. ( ( nTotFedCrg + nTotEstCrg + nTotMunCrg ) > 0 )
					...
					cString		+= ' / '+' Lei da Transparência conforme Lei Federal nº 12.741/12 '+'Valor Aproximado do(s) Tributo(s): '
			...
			Else
				...
				If cMvMsgTrib $ "1-3" .And. nTotalCrg > 0 .And. cTpCliente == "F"
					...
					If !Empty(cFntCtrb)
						...
						cString += ' / '+' Lei da Transparência conforme Lei Federal nº 12.741/12 '+'Valor Aproximado dos Tributos: R$ '+ConvType(nTotalCrg,15,2)+ " ("+cPercTrib+"%). Fonte: "+cFntCtrb+"."
					Else
						//steck
						//cString += 'Valor Aproximado dos Tributos: R$ ' +ConvType(nTotalCrg,15,2)+ " ("+cPercTrib+"%)."
						cString += ' / '+' Lei da Transparência conforme Lei Federal nº 12.741/12 '+'Valor Aproximado dos Tributos: R$ '+ConvType(nTotalCrg,15,2)+ " ("+cPercTrib+"%)."
					EndIf
				EndIf
...
//Tratamento para adcionar o valor do ICMS desonerado para informação complementar da Danfe.
			...
			// steck abre MSG COMPLEMENTAR ICMS 4%
			If _nRet = 1
				cString += "RESOLUCAO SF N.13/2012, ART. 1, CAPIT E INCISO 1. AJUSTE SINIEF N 19/2012, CLAUSULAS SEGUNDA E DECIMA PRIMEIRA"
			Endif
			_nRet := 0
			_cArea    := getArea()
			_cSD2Area := SD2->(GetArea())
			_cSd2Doc := SD2->D2_DOC
			__lCst	 := .F.
			DbSelectArea("SD2")
			DbSetOrder(3)
			DbSeek(xFilial("SD2")+_cSd2Doc)
			While !SD2->(Eof()) .And. _cSd2Doc == SD2->D2_DOC
				If SD2->D2_CLASFIS $ ("010#110#310#510")
					__lCst	:= .T.
				EndIf
				SD2->(DbSkip())
			EndDo
			SD2->(RestArea(_cSD2Area))
			RestArea(_cArea)
			If __lCst
				If SA1->A1_EST="SP"
					cString += "ICMS-ST ART.313-Y E 313-Z18 DO RICMS/SP DEC.N.52.921/2008 O DESTINATARIO DEVERA COM RELACAO AS OPERACOES COM MERCAD. OU PREST.SERV. RECEBIDAS COM IMP.RETIDO, ESCRITURAR O DOC. FISCAL NOS TERMOS DO ART.278 DO RICMS"
				ElseIf SA1->A1_EST="AL"
					cString += "ICMS/ST AL X SP - DECRETO No. 23.180/2012"
				ElseIf SA1->A1_EST="BA"
					cString += "ICMS/ST BA X SP - DECRETO No.14242/2012"
				ElseIf SA1->A1_EST="MG"
					cString += "ICMS/ST MG X SP - DECRETO No. 46.137/2013"
				ElseIf SA1->A1_EST="PE"
					cString += "ICMS/ST PE X SP  - DECRETO No.  35.678/2010"
				ElseIf SA1->A1_EST="RS"
					cString += "ICMS/ST RS X SP  - DECRETO No.  46.576/2009"
				ElseIf SA1->A1_EST="AP"
					cString += "ICMS/ST AP X SP - DECRETO No.  4046/2011"
				ElseIf SA1->A1_EST="DF"
					cString += "ICMS/ST DF X SP - DECRETO No. 33.999/2012"
				ElseIf SA1->A1_EST="PR"
					cString += "ICMS/ST PR X SP - DECRETO No. 3949/2012"
				ElseIf SA1->A1_EST="GO"
					cString += "ICMS/ST GO X SP - DECRETO No. 7528/2011"
				ElseIf SA1->A1_EST="MA"
					cString += "ICMS/ST MA X SP - RESOLUÇÃO ADM 08/2012 E 09/2012"
				ElseIf SA1->A1_EST="SE"
					cString += "ICMS/ST SE X SP - DECRETO Nº 28.199/2011"
				ElseIf SA1->A1_EST="SC"
					cString += "ICMS/ST SC X SP - DECRETO No.  3174/2010"
				EndIf
			EndIf
			// steck fecha MSG COMPLEMENTAR ICMS 4%
	...
	*/
					...
					If ( cTipo == "1") //Somente se nota de saída ou devolução.
						If !Empty(aExp[01][04][03])
							...
							// STECK ABRE
							//Giovani Zago 27/04/15 erro no fonte avgeral que nao retorna o array completo.
							//cString += NfeTag('<locdespacho>' ,ConvType(aExp[01][06]))
							DbSelectArea("EEC")
							EEC->(dbSetOrder(1)) // xFilial+EEC_PREEMB
							If EEC->(dbSeek(xFilial("EEC")+ Substr(SC5->C5_PEDEXP,1,6)))
								cString += NfeTag('<locdespacho>' ,Posicione("SJ0",1,xFilial("SJ0")+AvKey(EEC->EEC_URFDSP,"J0_COD"),"J0_DESC"))
								//Else
								//cString += NfeTag('<locdespacho>' ,ConvType(aExp[01][06]))
							EndIf
							//STECK FECHA ***********************************************************************************************
...
Static Function VldIE(cInsc,lContr,lIsent)
	...
	If lContr .And. Empty(cRet) .And. lIsent
		cRet := "ISENTO"
	EndIf
	//steck abre chamado 003242
	//If !lContr
	//	cRet := ""
	//EndIf
	//steck fecha chamado 003242
Return(cRet)
...
±±ºPrograma  ³STRETIMP	ºAutor  ³Renato Nogueira     º Data ³  08/08/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Fonte utilizado para retornar o imposto de produtos 		  º±±
±±º          ³importados da nota				  	 				      º±±
...
Static Function STRETIMP()
	Local cQuery 	:= ""
	Local cAlias 	:= "QRYTEMP"
	Local _nVal		:= 0

	cQuery  := " SELECT ROUND(SUM(D2_TOTAL*(7/100)),2) AS SUBVALOR "
	cQuery  += " FROM " +RetSqlName("SD2")+ " D2 "
	cQuery  += " WHERE D2.D_E_L_E_T_=' ' AND D2_FILIAL='"+SD2->D2_FILIAL+"' AND D2_DOC='"+SD2->D2_DOC+"' AND D2_SERIE='"+SD2->D2_SERIE+"' "
	cQuery  += " AND D2_CLIENTE='"+SD2->D2_CLIENTE+"' AND D2_LOJA='"+SD2->D2_LOJA+"' AND SUBSTR(D2_CLASFIS,1,1)='1' "
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³STXORDEM	ºAutor  ³Giovani Zago        º Data ³  24/08/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³utilizado para retornar o campo c6_xordem na tag infadprod  º±±
±±º          ³									  	 				      º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºParametro ³ Nenhum                                                     º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function STXORDEM(_cProdx)
	Local cQuery 	:= ""
	Local cAlias 	:= "STXORDEM"
	Local _cRetX	:= ' '


	cQuery  := " SELECT NVL(SC6.C6_XORDEM, ' ')
	cQuery  += ' "C6_XORDEM" , C6_NUM, C6_ITEM,C6_CLI
	...
	dbSelectArea(cAlias)
	(cAlias)->(dbGoTop())
	_cRetX	:= Alltrim((cAlias)->C6_XORDEM)
	If cEmpAnt = '03' .And. (cAlias)->C6_CLI = '012047'
		_cRetX += "-"+(cAlias)->C6_NUM+"-"+(cAlias)->C6_ITEM
	EndIf
Return(_cRetX)
...
/*/{Protheus.doc} GETORDSEP
@name GETORDSEP
@type Static Function
@desc retornar numero da OS
...
Static Function GETORDSEP()
	...
	_cQuery1 := " SELECT DISTINCT CB7_ORDSEP
	_cQuery1 += " FROM "+RetSqlName("CB7")+" CB7
	_cQuery1 += " WHERE CB7.D_E_L_E_T_=' ' AND CB7_FILIAL='"+SF2->F2_FILIAL+"'
	_cQuery1 += " AND CB7_NOTA='"+SF2->F2_DOC+"' AND CB7_SERIE='"+SF2->F2_SERIE+"'
	_cQuery1 += " ORDER BY CB7_ORDSEP DESC
