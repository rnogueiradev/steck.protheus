...
#DEFINE MAXITEMC   048                    //STECK                     // Máxima de caracteres por linha de produtos/serviços
...
/*/
//-------------------------------------------------------------------------------//
//FR - 15/08/2022 - ALTERAÇÃO - IMPRESSÃO ROMANEIO E DANFE PELA ROTINA STFATROM
//função auxiliar para chamar Danfeproc daqui de dentro 
//pois a chamada Danfeproc de dentro do fonte STFSF60 passou a não funcionar após
//a migração para release .33
//-------------------------------------------------------------------------------//
USER FUNCTION FDANFEPRO(oDanfe, lEnd, cIdEnt, cVal1, cVal2, lExistNfe, lIsLoja, nTipo)

	Default lIsLoja := .F.
	Default nTipo	:= 0

	DanfeProc(@oDanfe, @lEnd, cIdent, , , .F.)
		...
		While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
			...
			If  Empty((cAliasSF3)->F3_DTCANC) .And. &cWhere //.And. AModNot((cAliasSF3)->F3_ESPECIE)=="55"
				...
				If (SubStr((cAliasSF3)->F3_CFO,1,1)>="5" .Or. (cAliasSF3)->F3_FORMUL=="S") .And. aScan(aNotas,{|x| x[4]+x[5]+x[6]+x[7]==(cAliasSF3)->F3_SERIE+(cAliasSF3)->F3_NFISCAL+(cAliasSF3)->F3_CLIEFOR+(cAliasSF3)->F3_LOJA})==0
					If  NFEPDF((cAliasSF3)->F3_FILIAL,(cAliasSF3)->F3_SERIE,(cAliasSF3)->F3_NFISCAL)//steck giovani zago filtro na emissao de danfe 08/03/17 chamado: 005247
						aadd(aNotas,{})
						aadd(Atail(aNotas),.F.)
					...
					EndIf //steck giovani zago filtro na emissao de danfe 08/03/17 chamado: 005247
				EndIf
			EndIf
	...
	ElseIf  lImpDir
		...
		While  !lBreak  .And. nLenarray >= 0
			...
			While !Eof() .And. !lBreak .And. ;
				...
				aNotas := {}
				For nx:=1 To 20
					If    NFEPDF(cfilant,MV_PAR03, Alltrim(StrZero(nCursor,Len(MV_PAR01))))//steck giovani zago filtro na emissao de danfe 08/03/17 chamado: 005247
						aadd(aNotas,{})
						aAdd(Atail(aNotas),.F.)
...
Static Function ImpDet(	oDanfe		, oNfe			, cCodAutSef	, cModalidade	,;
	...
	If ntipo <> 1
		...
		PRIVATE oFont08    := TFontEx():New(oDanfe,"Times New Roman",06,06,.F.,.T.,.F.)// 4//everson
		...
		PRIVATE OFONT08S   := TFontEx():New(oDanfe,"Arial Black"    ,07,07,.T.,.F.,.F.)// 13 //Steck - Renato Nogueira - Destacar endereço de entrega
		PRIVATE OFONT07S   := TFontEx():New(oDanfe,"Arial Black"    ,06,06,.T.,.F.,.F.)// 13 //Steck - GIOVANI ZAGO - Destacar endereço de entrega
		PRIVATE oFont16N   := TFontEx():New(oDanfe,"Times New Roman",15,15,.T.,.T.,.F.)// 10
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	Local cDhCont		:= ""
	Local cXJust		:= ""
	Local kx            := 0 //Giovani.Zago 12/06/13 - steck
	...
	Private _xCodCli  := ''   //GIOVANI ZAGO PROJETO ONDA 2  07/02/2013 GAP 09-10-11-12 - steck
	...
	If(MV_PAR04==2) .And. Empty(aTransp[12])
		...
		If Empty(SF2->F2_ESPECI1) //Steck - Não estava saindo peso e volume em algumas notas
			SF2->(Reclock("SF2",.F.))
			SF2->F2_ESPECI1	:= "CX"
			SF2->(MsUnlock())
		EndIF
	...
	For nZ := 1 To nLenDet
		...
		_cItem := IIf(Len(oDet[nX]:_NITEM:TEXT) > 1,oDet[nX]:_NITEM:TEXT,"0" + oDet[nX]:_NITEM:TEXT)

		//GIOVANI ZAGO PROJETO ONDA 2  07/02/2013 GAP 09-10-11-12 - steck
		_xCodCli := oDet[nX]:_Prod:_xProd:TEXT
		//Chamado 007298 - Everson Santana - 26.04.18
		If SF2->F2_CLIENTE = '012047'
			If SF2->F2_EMISSAO>=CTOD("26/04/2018")
				If type("oDet[nX]:_Prod:_nitemped:TEXT") <> 'U'
					_cItem := oDet[nX]:_Prod:_nitemped:TEXT
				EndIf
			EndIf
		EndIf

		If(MV_PAR04==2)//Somente entrar nesta rotina se for nota fiscal de saida. Everson Santana - 08/0519
			_xCodCli += StGap09(SF2->F2_DOC,SF2->F2_SERIE,oDet[nX]:_Prod:_cProd:TEXT,_cItem)
		EndIF
		//****************************************************************

		aadd(aItens,{;
			SubStr(oDet[nX]:_Prod:_cProd:TEXT,1,nMaxCod),;
			SubStr(NoChar(_xCodCli),1,nMaxDes),;  //GIOVANI ZAGO PROJETO ONDA 2  07/02/2013 GAP 09-10-11-12 - steck
	...
	//GIOVANI ZAGO PROJETO ONDA 2  07/02/2013 GAP 09-10-11-12 - steck
	If !(Empty(alltrim(  Posicione("SC5",1,xFilial("SC5")+Posicione("SC6",4,xFilial("SC6")+SF2->F2_DOC+SF2->F2_SERIE,"C6_NUM"),"C5_XORDEM"))))
		cAux := "ORDEM DE COMPRA Nº: " +alltrim(Posicione("SC5",1,xFilial("SC5")+Posicione("SC6",4,xFilial("SC6")+SF2->F2_DOC+SF2->F2_SERIE,"C6_NUM"),"C5_XORDEM"))
		While !Empty(cAux)
			aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
			cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
		EndDo
	...
	//jackson - chamado 20230511005962
	if SF2->F2_XCUBAGE > 0
		aadd(aMensagem,'Cubagem : '+TRANSFORM( SF2->F2_XCUBAGE, "@E 999,999.99" ))
	ENDIF
	//****************************************************************************************************************

	//Giovani Zago Regiao de entrega  18/05/13 - steck abre
	If !(cEmpAnt = '03')
		cAux := "Região:  "+U_StCepReg(SF2->F2_DOC,SF2->F2_SERIE)+"         "
	EndIf
	//>>Chamado 006963 Everson Santana 01.03.18
	_cAux1 := ""
	_zObs  := ""
	_cLacre := ""
	//<<Chamado 006963
	If oIdent:_TpNf:Text="1"
		...
		If SD2->(dbseek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE))
			If SD2->D2_CF <> '7949'
				//cAux += " Pedido de Venda: "
				cAux += " Pedido: " //Ticket 20190823000028
				dbselectarea("SC9")
				SC9->(dbsetorder(6))
				If SC9->(dbseek(xFilial("SC9")+SF2->F2_SERIE+SF2->F2_DOC))
					While SC9->(!Eof()) .And. xFilial("SC9")+SF2->F2_SERIE+SF2->F2_DOC = SC9->(C9_FILIAL+C9_SERIENF+C9_NFISCAL)
						If At(SC9->C9_PEDIDO,cAux)=0
							//>> Chamado 006963 Everson Santana 01.03.18
							_zObs := alltrim(Posicione("SC5",1,xFilial("SC5")+SC9->C9_PEDIDO,"C5_ZOBS"))
							...
							If !Empty(_zObs)
								...
								EndIf
							EndIf
							_cLacre := Alltrim(Posicione("ZZT",2,xFilial("ZZT")+SC9->C9_NFISCAL,"ZZT_OBSNF"))
							If !Empty(_cLacre)
								If Empty(_cAux1)
									_cAux1 := _cLacre
								Else
							...
							If kx = 0
								cAux += SC9->C9_PEDIDO
							Else
								cAux += Iif(Len(cAux)>17,", ","")+SC9->C9_PEDIDO

							EndIf
							kx++
							_cLacre := ""//Ticket  20190820000008
							_zObs := "" //>>Chamado 006963 Everson Santana 01.03.18
						EndIf
						SC9->(dbskip())
					Enddo
				EndIf
				//cAux += "   OBS:  "+ _cAux1 //>>Chamado 006963 Everson Santana 01.03.18
				kx:=0
			EndIf
		EndIf
		//cAux += "         Ordem de Separacao: "

		cAux += "         O.S.: " //Ticket 20190823000028
		dbselectarea("CB7")
		CB7->(dbsetorder(4))
		If CB7->(dbseek(xFilial("CB7")+SF2->F2_DOC+SF2->F2_SERIE))
			While CB7->(!Eof()) .And. xFilial("CB7")+SF2->F2_DOC+SF2->F2_SERIE = CB7->(CB7_FILIAL+CB7_NOTA+CB7_SERIE)
				If kx = 0
					cAux += CB7->CB7_ORDSEP
				Else
					cAux += Iif(Len(cAux)>18,", ","")+CB7->CB7_ORDSEP
				EndIf
				kx++
	...
	If !Empty(_cAux1)
		cAux += "                           OBSERVAÇÃO:  " + _cAux1 //>>Chamado 006963 Everson Santana 01.03.18
	EndIF

	If GetMv("STCDARUJA",,.T.) .And. AllTrim(SF2->F2_FILIAL)=="02"
		cAux += " - Local de Retida da Mercadoria: Avenida Adilia Barbosa Neves, nº.3925, Módulos B3 e B4, Portão, cidade de Arujá, Estado de São Paulo, CEP: 07413-00"
	EndIf

	/*If SF2->F2_CLIENTE=="057343"
		...
		cAux += "MERCADORIAS À SEREM ENTREGUES NO DEPÓSITO FECHADO "
		cAux += "MJR CUNHA DISTRIBUIDORA DE MATERIAIS PARA CONSTRUÇÃO LTDA "
		cAux += "AVENIDA FABER, Nº 555, - DISTRITO INDUSTRIAL MIGUEL ABDELNUR "
		cAux += "CEP: 13571-370 - SÃO CARLOS - SP - FONE (16) 3368-1428 "
		cAux += "CNPJ 13.571.969/0002-45 - INSCRIÇÃO ESTADUAL 637.208.073.110 "
		cAux += "ENDEREÇO PARA COBRANÇA: ESTRADA MUNICIPAL, 155 - JARDIM SANTO AFONSO "
		cAux += "CEP: 07215-040 - GUARULHOS - SP "
		cAux += "CONFORME ANEXO VII REGULAMENTO "
		cAux += "ICMS DECRETO 45490 / 2000 "
	...
	If !Empty(SC5->C5_XNUMWEB)
		cAux += "	Ped. internet:  "+AllTrim(SC5->C5_XWEB)
	EndIf


	//JOÃO VICTOR PROJETO ONDA 2  27/02/2013 - SOLICITACAO DE TATIANA NEVES E VIVIANE DIAS
	/*//>>Chamado 006963 Everson Santana 01.03.18 - Retirei para realizar a validação por Pedido.
	If !(Empty(alltrim(  Posicione("SC5",1,xFilial("SC5")+Posicione("SC6",4,xFilial("SC6")+SF2->F2_DOC+SF2->F2_SERIE,"C6_NUM"),"C5_ZOBS"))))
	cAux := "OBS: " +alltrim(Posicione("SC5",1,xFilial("SC5")+Posicione("SC6",4,xFilial("SC6")+SF2->F2_DOC+SF2->F2_SERIE,"C6_NUM"),"C5_ZOBS"))
	While !Empty(cAux)
	aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
	cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
	EndDo
	...
	*///>>Chamado 006963
	...
	If !Empty(SA1->A1_FORMVIS)
		...
		If !SM4->(Eof())
			cAux	:= AllTrim(SM4->M4_DESCR)+": "+AllTrim(StrTran(SM4->M4_FORMULA,'"',""))
		EndIf
	EndIf
	While !Empty(cAux)
		aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
	...
	*/
	...
	oDanfe:Say(052,105, "Identificação do emitente",oFont12N:oFont) //de 098 para 105
	nLinCalc	:=	065
	...
	For nX := 1 To nForTo
		oDanfe:Say(nLinCalc,105,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*25)+1),25), oFont12N:oFont )
		nLinCalc+=10
	Next nX
	...
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	For nX := 1 To nForTo
		oDanfe:Say(nLinCalc,105,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*40)+1),40),oFont08N:oFont)
		nLinCalc+=10
	Next nX
	...
	If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
		...
		For nX := 1 To nForTo
			...
			nLinCalc+=10
		Next nX
		oDanfe:Say(nLinCalc,105, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
		nLinCalc+=10
		oDanfe:Say(nLinCalc,105, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
	Else
		oDanfe:Say(nLinCalc,105, NoChar(oEmitente:_EnderEmit:_xBairro:Text,lConverte)+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont)
		nLinCalc+=10
		oDanfe:Say(nLinCalc,105, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
		nLinCalc+=10
		oDanfe:Say(nLinCalc,105, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
	...
	If nfolha==1
		If lMv_Logod
			...
			//oDanfe:SayBitmap(072,001,cLogoD,093,026) //steck
			oDanfe:SayBitmap(072,001,cLogoD,097,019) //steck Novo
		Else
	...
	//steck abre - adicionar tipo de entrada nas informações da transportadora
	...
	*/
	...
	oDanfe:Say(372,317,"TP ENTREGA",oFont08N:oFont)
	//If SC5->C5_XTIPO="1" GIOVANI ZAGO PERDEU O POSICIONAMENTO 23/06/2017
	If  Alltrim(Posicione("SC5",1,xFilial("SC5")+SD2->D2_PEDIDO,"C5_XTIPO"))="1"
		oDanfe:Say(382,319,"RETIRA",oFont08:oFont)
		//ElseIf ="2"
	ElseIf Alltrim(Posicione("SC5",1,xFilial("SC5")+SD2->D2_PEDIDO,"C5_XTIPO"))="2"
		oDanfe:Say(382,319,"ENTREGA",oFont08:oFont)
	EndIf
	...
	//steck fecha - adicionar tipo de entrada nas informações da transportadora

	oDanfe:Box(363,370,386,490)
	oDanfe:Say(372,375,"BAIRRO",oFont08N:oFont) //Steck
	oDanfe:Say(382,375,SA4->A4_BAIRRO,oFont08:oFont) //Steck
	...
	For nX := 1 To Min(nLenMensagens, MAXMSG)
		If "DE ENTREGA" $ aMensagem[nX]  //Steck - Renato Nogueira - Destacar endereço de entrega - abre
			oDanfe:Say(nLin,002,aMensagem[nX],OFONT07S:oFont)
		ElseIf (" Pedido: " $ aMensagem[nX])          	//ticket Nº 20190823000028 - INICIO
			oDanfe:Say(nLin,002,aMensagem[nX],oFont16N:oFont)	//ticket Nº 20190823000028 - FIM
		Else
			oDanfe:Say(nLin,002,aMensagem[nX],oFont07:oFont)
		EndIf							//Steck - Renato Nogueira - Destacar endereço de entrega - fecha
		nLin:= nLin+10
	Next nX
	nMensagem := nX

	oDanfe:Box(721,352,865,603) //Steck
	...
	For nY := 1 To nLenItens
		...
		If lflag
			If nL > nMaxItemP2
				...
				oDanfe:Say(010+nLinhavers,105, "Identificação do emitente",oFont12N:oFont)
				...
				If lMv_Logod
					//oDanfe:SayBitmap(002+nLinhavers,002,cLogoD,085,085)
					oDanfe:SayBitmap(035+nLinhavers,002,cLogoD,097,019) //Steck Novo (Moura)
				Else
					//oDanfe:SayBitmap(002+nLinhavers,002,cLogo,085,085)
					oDanfe:SayBitmap(035+nLinhavers,002,cLogoD,097,019) //Steck Novo
		...
		If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
			...
			oDanfe:Say(010,105, "Identificação do emitente",oFont12N:oFont)
			nLinCalc	:=	023
			cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
			nForTo		:=	Len(cStrAux)/25
			nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
			...
			If lMv_Logod
				...
				//oDanfe:SayBitmap(072,001,cLogoD,093,026) //steck
				oDanfe:SayBitmap(072,001,cLogoD,097,019) //steck
			Else
...
Static Function DanfeCpl(oDanfe,aItens,aMensagem,nItem,nMensagem,oNFe,oIdent,oEmitente,nFolha,nFolhas,cCodAutSef,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,cLogoD,aUF)
	...
	If lMensagens
		...
		If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
			...
			oDanfe:Say(nLinCalc,105, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
		Else
			oDanfe:Say(nLinCalc,105, oEmitente:_EnderEmit:_xBairro:Text+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont)
			nLinCalc+=10
			oDanfe:Say(nLinCalc,105, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
			nLinCalc+=10
			oDanfe:Say(nLinCalc,105, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
...
Static Function SimpDanfe(oDanfe,oNfe,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	oDanfe:Say(052,105, "Identificação do emitente",oFont12N:oFont)
	nLinCalc	:=	065
	cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
	nForTo		:=	Len(cStrAux)/25
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	If lMv_Logod
		cLogoD := GetSrvProfString("Startpath","") + "STECK" + cEmpAnt + cFilAnt + ".BMP"
		If !File(cLogoD)
			cLogoD	:= GetSrvProfString("Startpath","") + "STECK" + cEmpAnt + ".BMP"
			If !File(cLogoD)
				lMv_Logod := .F.
...
//GIOVANI ZAGO PROJETO ONDA 2  07/02/2013 GAP 09-10-11-12 - steck
Static Function StGap09(_xCdoc,_xCserie,_xCprod,_cItem)
	Local _cRet    := ''
	...
	DbSelectArea("SC6")
	SC6->(DbSetOrder(4))
	If SC6->(DbSeek(xFilial("SC6")+_xCdoc+_xCserie))
		While 	SC6->(!EOF()) .And. SC6->C6_NOTA = _xCdoc .And. SC6->C6_SERIE = _xCserie
			If Alltrim(SC6->C6_PRODUTO) = Alltrim(_xCprod) //.AND. SC6->C6_ITEM = _cItem

				If AllTrim(SC6->C6_ITEM) == AllTrim(_cItem)
					_cAuxX := " "+Alltrim(SC6->C6_XORDEM)
				Else
					_cRet := " "+Alltrim(SC6->C6_XORDEM)
				EndIf
				If SC6->C6_CLI = '012047'
					_cRet += "/"+AllTrim(_cItem)
					_cAuxX += "/"+AllTrim(_cItem)
				EndIf
			EndIf
			SC6->(DbSkip())
		End
...
//steck giovani zago filtro na emissao de danfe 08/03/17 chamado: 005247
Static Function NFEPDF(_cFil,_cSerie,_cNf)

	Local _lPdfRet 	:= .f.
	Local cQuery 	:= ""
	Local cAlias 	:= "NFEPDF"

	//If (GetMv("ST_NFEPDF",,.T.))
	//Return(.T.)
	//EndIf
	Return(.T.)
	cQuery	:= " SELECT F2_VEND1 FROM SF2010 SF2
	cQuery	+= " WHERE SF2.D_E_L_E_T_ = ' '
	cQuery	+= " AND F2_DOC = '"+ _cNf +" '
	cQuery	+= " AND F2_SERIE = '"+ _cSerie +" '
	cQuery	+= " AND F2_FILIAL = '"+ _cFil +" '
	cQuery	+= " AND F2_VEND1 = 'R00016'
...
//Steck - Imprimir danfe e guardar em pasta
/*====================================================================================\
|Programa  | STXPRTNF        | Autor | RENATO.OLIVEIRA           | Data | 10/09/2018  |
|=====================================================================================|
|Descrição | ROTINA PARA IMPRIMIR DANFE				                                  |
|          |                                                                          |
|          |                                                                          |
|=====================================================================================|
|Sintaxe   | 		                                                                  |
...
User Function STXPRTNF(cChave,_lJob)

	Local aArea     	:= GetArea()
	Local lExistNfe 	:= .F.
	Local cFile      	:= AllTrim(cChave)//+".pdf"
	Local oDanfe 		:= FwMSPrinter():New(cFile+".rel" , IMP_PDF , .F. , "\arquivos\xml_nfe\"+cEmpAnt+"\", .T., , , , .F., .F., ,.F. )
	Local cIdEnt      	:= ""
	Local _cQuery1 		:= ""
	...
	oDanfe:SetPaperSize(DMPAPER_A4)
	oDanfe:SetMargin(60,60,60,60)
	oDanfe:cPathPDF:= "\arquivos\xml_nfe\"+cEmpAnt+"\"
	PixelX := oDanfe:nLogPixelX()
	PixelY := oDanfe:nLogPixelY()

	_cQuery1 := " SELECT F2.R_E_C_N_O_ RECSF2
	_cQuery1 += " FROM "+RetSqlName("SF2")+" F2
	_cQuery1 += " WHERE F2.D_E_L_E_T_=' ' AND F2_CHVNFE='"+cChave+"'
	...
	If (_cAlias1)->(!Eof())
		...
		SF2->(DbGoTo((_cAlias1)->RECSF2))

		If SF2->(Eof())
			conout("Chave não encontrada " + cChave)
			Return
		EndIf
	...
	if !SD2->(dbSeek(SF2->(F2_FILIAL+F2_DOC+F2_SERIE+F2_CLIENTE+F2_LOJA)))
		conout("Itens não encontrado " + cChave)
		return
	endif
	...
	if !SC5->(dbSeek(SD2->(D2_FILIAL+D2_PEDIDO)))
		conout("Pedido não encontrado " + cChave)
		return
	endif

	dbSelectArea("SM0")
	dbsetOrder(1)
	if SM0->(dbSeek(cEmpAnt+SF2->F2_FILIAL))
		cIdEnt := U_XGETIND(SM0->M0_CGC,"1")
		cFilAnt := PADR(SM0->M0_CODFIL,TamSx3("F2_FILIAL")[1])
	endif
	...
	If !_lJob
		...
		//If FTPUpLoad(cDir,cDirFtp+cChave+".pdf")
		//	Conout('UpLoad Ok! ')
		//Else
		//	Conout('Falha UpLoad!')
		//EndIf
...
//Steck - Imprimir danfe e mostrar na tela do usuario
/*====================================================================================\
|Programa  | STRONFE        | Autor | GIOVANI ZAGO           | Data | 10/09/2018  |
|=====================================================================================|
|Descrição | ROTINA PARA IMPRIMIR DANFE				                                  |
|          |                                                                          |
|          |                                                                          |
...
User Function STRONFE(cChave,_lJob,_cDoc)
	...
	oDanfe 		:= FwMSPrinter():New(cFile+".rel" , IMP_PDF , .F. , "\arquivos\xml_nfe\"+cEmpAnt+"\", .T., , , , .F., .F., ,.F. )
	...
	If !Empty(cIdEnt)
		...
		If lExistNfe
			...
			If IsInCallStack("U_STSIMPV")
				CpyS2T("\arquivos\xml_nfe\"+cEmpAnt+"\"+SF2->F2_DOC+".pdf",AllTrim(Getmv("MV_RELT",,'C:\ARQUIVOS_PROTHEUS\'))+"\",.T.) // COPIA ARQUIVO PARA MAQUINA DO USUÁRIO
				ShellExecute("open",AllTrim(Getmv("MV_RELT",,'C:\ARQUIVOS_PROTHEUS\'))+"\"+SF2->F2_DOC+".pdf", "", "", 1)
			EndIf
		Else
			_naoimprime := .t.
		EndIf
	...
	If !_lJob
		...
		cDir 	:= GetMv("ST_FTPDIR1",,"\arquivos\xml_nfe\"+cEmpAnt+"\"+cChave+".pdf")
		cServer	:= GetMv("ST_FTPSRV1",,"www.appstk.com.br")
		cUser	:= GetMv("ST_FTPUSR1",,"renato.oliveira")
		cPass	:= GetMv("ST_FTPPSW1",,"steck@2018")
		cDirFtp	:= GetMv("ST_FTPDIR1",,"/portal_cliente/notas/")
