...
User Function STFIN021()

	U_STFIN020("11","01")

Return()

User Function STFIN022()

	U_STFIN020("01","02")

Return()

User Function STFIN020(_cEmp,_cFil)
	...
	//Códigos de clientes Steck do Brasil: 
	_aEmpStk := StrTokArr(GetMv("STDSOCLIST",,"035444#100557#033467#012047#000042"),"#")
	...
	//MemoWrite("C:\QUERY\ITEMS.TXT" , _cQuery1)
	...
	_cArqItems  := "items_" + Dtos(Date()) + ".txt"
	...
	*/
	...
	_cQuery1 += " 	A1M.A1_XSEGVAL,
	_cQuery1 += " 	A1.A1_RISCO,
	_cQuery1 += " 	A1.A1_CGC,
	_cQuery1 += " 	SC9.SALPED A1_SALPED,
	_cQuery1 += " 	A1.A1_COND,
	_cQuery1 += " 	A1.A1_DDD||A1.A1_TEL TEL2,
	_cQuery1 += " 	A1.A1_NREDUZ,
	_cQuery1 += " 	A1.A1_HPAGE,
	_cQuery1 += " 	A1.A1_XCOBRAD,
	//_cQuery1 += " 	RTRIM(A1.A1_GRPVEN)||'-'||ACY_DESCRI A1_GRPVEN,
	//>>Ticket 20221027019831 - Everson Santana - 27.10.2022
	_cQuery1 += " 	ACY_XCHANN A1_GRPVEN, 
	//<<
	...
	_cQuery1 += " 	A1.A1_XAVSEML,
	_cQuery1 += " 	A1.A1_XBLOQF
	_cQuery1 += " FROM " + RetSqlName("SE1") + " E1 " 

	_cQuery1 += " LEFT JOIN " + RetSqlName("SA1") + " A1
	_cQuery1 += " 	ON A1.A1_COD = E1_CLIENTE AND A1.A1_LOJA = '01' AND A1.D_E_L_E_T_ = ' ' 
	_cQuery1 += "   AND A1.A1_COD NOT IN (" + _cEmpStk + ") "
	...
	_cQuery1 += "	LEFT JOIN (
	_cQuery1 += "	SELECT C9_CLIENTE,SUM((C9_QTDLIB * C9_PRCVEN )) SALPED FROM "+RetSqlName("SC9")
	_cQuery1 += " WHERE C9_QTDLIB > 0
	_cQuery1 += " AND C9_NFISCAL = ' '
	_cQuery1 += " and D_E_L_E_T_ = ' '
	_cQuery1 += " GROUP BY C9_CLIENTE
	_cQuery1 += " ) SC9 
	_cQuery1 += " ON SC9.C9_CLIENTE = E1_CLIENTE
	_cQuery1 += " WHERE E1.D_E_L_E_T_ = ' '
	_cQuery1 += " 	AND E1_FILIAL = '" + cNewFil + "'
	_cQuery1 += " 	AND E1_SALDO > 0
	_cQuery1 += " 	AND A1.A1_TIPO NOT IN ('X') "
	//_cQuery1 += " AND e1_cliente = '003322' "
	_cQuery1 += " 	AND E1_CLIENTE NOT IN (" + _cEmpStk + ") "
	

	//MemoWrite("C:\QUERY\CLIENTS.TXT" , _cQuery1)
	...
	While (_cAlias1)->( !Eof() )
		...
		_cClients += '"' + AllTrim((_cAlias1)->CCH_PAIS) + '";'
	...
	_cArqClient := "clients_" + Dtos(Date()) + ".txt"
