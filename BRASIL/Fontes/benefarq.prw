...
If lNovoCalc
	...
	cSelect  += " RA_ENDEREC,RA_COMPLEM,RA_BAIRRO,RA_MUNICIP,RA_ESTADO,RA_CEP,RA_TELEFON,R0_NROPED, "
	cSelect  += " RA_NASC,RA_CIC,RA_RG,RA_MAE,RA_EMAIL,RA_ESTCIVI,RA_SEXO,RA_PAI,RA_VALEALI,RA_RGUF,R0_VALCAL,RA_DTRGEXP,RA_LOGRTP,RA_NUMENDE "
...
Else
	...
	cQuery  += " RA_ENDEREC,RA_COMPLEM,RA_BAIRRO,RA_MUNICIP,RA_ESTADO,RA_CEP,RA_TELEFON,R0_NROPED, "
	cQuery  += " RA_NASC,RA_CIC,RA_RG,RA_MAE,RA_EMAIL,RA_ESTCIVI,RA_SEXO,RA_PAI,RA_VALEALI,RA_RGUF,R0_VALCAL,RA_DTRGEXP,RA_LOGRTP,RA_NUMENDE "
...
//Verifica Tabela Aberta
If Select("QD02SDX") > 0
	DbSelectArea("QD02SDX")
	DbCloseArea()
Endif

//Abrir Tabela
DbUseArea( .T., "TOPCONN", TcGenQry( ,, cQuery), "QD02SDX", .T., .F.)

//Atualiza regua
oProcess:SetRegua2(QD02SDX->(RecCount()))
oProcess:IncRegua2("")
QD02SDX->(DbGoTop())
...
For nAux := 1 To Len(aSraStruct)
	If ( aSraStruct[nAux][2] <> "C" )
		TcSetField("QD02SDX",aSraStruct[nAux][1],aSraStruct[nAux][2],aSraStruct[nAux][3],aSraStruct[nAux][4])
	EndIf
Next nAux
QD02SDX->(DbGoTop())
...
While QD02SDX->(!Eof())
	...
	If QD02SDX->(RA_FILIAL+RA_MAT) == cMyChave
		QD02SDX->(dbSkip())
		Loop
	Else
		cMyChave := QD02SDX->(RA_FILIAL+RA_MAT)
	EndIf
	...
	cSRA_End := ''
	cSRA_Num := ''
	If (nPos := Rat(",",QD02SDX->RA_ENDEREC,1,1)) > 0
		cSRA_End := Substr(QD02SDX->RA_ENDEREC,1,nPos - 1)
		cSRA_Num := Substr(QD02SDX->RA_ENDEREC,nPos + 1)
	Endif
	If Empty(cSRA_END)
		cSRA_End := QD02SDX->RA_ENDEREC
	    cSRA_Num := "0"
	EndIf
	
	
	//-Busca a Sequencia do Endereco
	aAdd(aSeqEnd, {QD02SDX->RA_LOCBNF, nSeqEnd})
	nPosEnd := aScan(aSeqEnd, {|x| x[1]==QD02SDX->RA_LOCBNF })
...
While QD02SDX->(!Eof())
    ...
    If lNovoCalc
		...
		dbSelectArea("RGC")
		dbSetOrder(1)	//-RGC_FILIAL+RGC_KEYLOC
		dbSeek(xFilial("RGC")+QD02SDX->RA_LOCBNF,.F.)
    	...
    	oProcess:IncRegua2(QD02SDX->RA_NOME)
    	//--??fValidVB(oProcess, "QD02SDX")
		bAux := &( '{ || ' + Alltrim(cFuncVal) + '(oProcess, "QD02SDX") } ' )
		EVal( bAux )
		...
		Else
			cQuery += " SR0.@DELETED@ = ' ' "
		Endif
		cQuery  += " AND R0_FILIAL = '" + QD02SDX->RA_FILIAL + "'"
		cQuery  += " AND R0_CC =  '" + QD02SDX->RA_CC + "'"
		cQuery  += " AND R0_MAT =  '" + QD02SDX->RA_MAT + "'"
		...
		EndDo
		QD05VB->(DbCloseArea())
		aadd(aItens,{QD02SDX->RA_FILIAL+QD02SDX->RA_MAT,aTemp})
    Else
		...
		If QD02SDX->RG2_TPVALE = "0"
			dbSelectArea("SRN")
			dbSetOrder(1)	//-RN_FILIAL+RN_COD
			dbSeek(xFilial("SRN")+QD02SDX->RG2_CODIGO,.F.)
		...
		Else
			dbSelectArea("RFO")
			dbSetOrder(1)	//-RFO_FILIAL+RFO_TPVALE+RFO_CODIGO
			dbSeek(xFilial("RFO")+QD02SDX->RG2_TPVALE+QD02SDX->RG2_CODIGO,.F.)\
...
Private cPerg    := ''
Private wCabec0  := 2
Private wCabec1  := STR0042 +space(02) +STR0043 +space(04) +STR0044 +space(30) +STR0045 +space(5) +STR0046+space(5) +"Nro pedido"
...
If nOrd == 2
	...
	dbSeek(xFilial("CTT")+QD02SDX->RA_CC,.F.)
	
	cDet := Space(5) + AllTrim(QD02SDX->RA_CC) + " - " + CTT->CTT_DESC01
	Impr(cDet,'C')
Endif

//Carrega Filial
If lNovoCalc
	cFilialAnt := QD02SDX->RA_FILIAL
Else
	cFilialAnt := QD02SDX->RG2_FILIAL
EndIf

While !QD02SDX->(Eof())

	If lNovoCalc
		cFilReg := QD02SDX->RA_FILIAL
	Else
		cFilReg := QD02SDX->RG2_FILIAL
	EndIf
	...
	If lNovoCalc
		nPos := Ascan(aItens,{|x| x[1]==QD02SDX->RA_FILIAL+QD02SDX->RA_MAT})
		For nI := 1 To Len(aItens[nPos][2])
			...
			cDet := QD02SDX->RA_FILIAL + Space(4) + QD02SDX->RA_MAT + Space(4) + QD02SDX->RA_NOME + Space(10) + aItens[nPos][2][nI][1] + Space(9) +Transform(nVlr,'@E 999,999.99')+ Space(10) + QD02SDX->R0_NROPED
			Impr(cDet,'C')
		Next nI
	Else
		nVlr := QD02SDX->RG2_VALCAL
		cDet := QD02SDX->RG2_FILIAL + Space(4) + QD02SDX->RG2_MAT + Space(4) + QD02SDX->RA_NOME + Space(10) + QD02SDX->RG2_TPBEN + Space(9) +Transform(nVlr,'@E 999,999.99')+ Space(10) + QD02SDX->R0_NROPED
		Impr(cDet,'C')
	EndIf
	QD02SDX->(dbSkip())
	...
	If nOrd == 2
		If cCcAnt != QD02SDX->RA_CC .Or. cFilialAnt != cFilReg
			cCcAnt := QD02SDX->RA_CC
			...
			If !QD02SDX->(Eof()) .And. cFilialAnt == cFilReg
				dbSelectArea("CTT")
				dbSetOrder(1)	//-CTT_FILIAL+CTT_CUSTO
				dbSeek(xFilial("CTT")+QD02SDX->RA_CC,.F.)
...
Static Function fAtuRG2()
  ...
  While QD02SDX->(!Eof())
     ...
     If dbSeek( QD02SDX->RG2_FILIAL + QD02SDX->RG2_MAT + QD02SDX->RG2_ANOMES + QD02SDX->RG2_TPVALE + QD02SDX->RG2_CODIGO )
         RecLock("RG2",.F.)
         RG2->RG2_PEDIDO := 2
         MsUnlock()
     EndIf
