...
User Function MXFIN020()
	...
	//códigos de clientes Steck do México:
	_aEmpStk := StrTokArr(GetMv("STDSOCLIST",,"C03415#C02436#C03422#C03423#C05712#C05736#C05737#C06042#C06059"),"#")
	...
	MemoWrite("C:\QUERY\MEX_ITEMS.TXT" , _cQuery1)
	...
	_cArqItems  := "items_" + Dtos(Date()) + ".txt"      // Removido mex_ por estar apresentando problemas - Valdemir Rabelo 04/01/2023
...
//	_cQuery1 += " 	ON CCH_CODIGO = A1.A1_CODPAIS AND CCH.D_E_L_E_T_ = ' '
	...
	MemoWrite("C:\QUERY\MEX_CLIENTS.TXT" , _cQuery1)
	...
	While (_cAlias1)->( !Eof() )
		...
		****************************************************/
		IF AllTrim((_cAlias1)->A1_MSBLQL) <> "1"
			//_cClients += '"1";'
			_cClients += '"0";'		//FR - 07/12/20220 - Ticket #20221006018802 - David Souza disse que precisa ser 0 para clientes liberados
		ELSE
			//_cClients += '"0";'
			_cClients += '"1";'		//FR - 07/12/20220 - Ticket #20221006018802 - David Souza disse que precisa ser 1 para clientes bloqueados
		ENDIF		
	...
	_cArqClient := "clients_" + Dtos(Date()) + ".txt"    // Removido mex_ por estar apresentando problemas - Valdemir Rabelo 04/01/2023
